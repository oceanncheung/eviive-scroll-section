...
const DRIVER = "window.__eviiveStop&&window.__eviiveStop();let __eviiveDead=!1;window.__eviiveStop=function(){__eviiveDead=!0},(()=>{\"use strict\";const It=matchMedia(\"(prefers-reduced-motion: reduce)\").matches,g=(t,n,e)=>t<n?n:t>e?e:t,M=(t,n,e)=>t+(n-t)*e,k=(t,n,e)=>{const o=g((e-t)/(n-t),0,1);return o*o*(3-2*o)},Tt=6.4,Dt=53.5,a=window.__eviiveScroll=window.__eviiveScroll||{p:0};a.mx=a.my=0,addEventListener(\"pointermove\",t=>{a.mx=t.clientX/__eviiveVP.w*2-1,a.my=1-t.clientY/__eviiveVP.h*2},{passive:!0});const Ht=document.getElementById(\"pin\"),D=document.getElementById(\"ui\"),H=document.getElementById(\"line2\"),q=document.getElementById(\"l1a\"),W=document.getElementById(\"l1b\"),m=document.getElementById(\"caption\"),qt=document.getElementById(\"cue\"),xt=document.getElementById(\"hud\"),z=document.getElementById(\"odo\"),Wt=[{key:\"tens\",mul:.1},{key:\"ones\",mul:1},{key:\"point\"},{key:\"tenths\",mul:10}],Y={},st={},O={};let it=102;for(const t of Wt){if(t.key===\"point\"){const o=document.createElement(\"div\");o.className=\"odo-pt\",o.textContent=\".\",z.appendChild(o);continue}const n=document.createElement(\"div\");n.className=\"odo-col\";const e=document.createElement(\"div\");e.className=\"stack\";for(let o=0;o<=44;o++){const i=document.createElement(\"i\");i.textContent=String(o%10),e.appendChild(i)}n.appendChild(e),z.appendChild(n),Y[t.key]={st:e,col:n,mul:t.mul},O[t.key]=0}function ct(){Y.ones&&(it=Y.ones.col.getBoundingClientRect().height||it)}ct(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(ct),addEventListener(\"resize\",ct,{passive:!0});const Yt={tens:[0,0,5],ones:[0,6,23],tenths:[0,14,35]};function Ot(t,n){const e=Yt[t];return n<=.5?M(e[0],e[1],n/.5):M(e[1],e[2],(n-.5)/.5)}function Gt(t){for(const n in Y){const{st:e,col:o}=Y[n],i=Ot(n,t);e.style.transform=`translateY(${(.275-i).toFixed(4)}em)`;const l=st[n]===void 0?i:st[n];st[n]=i;const s=Math.abs(i-l);O[n]=O[n]*.84+s*.16;const p=g(O[n]*30,0,1);o.style.setProperty(\"--eat\",(p*11.5).toFixed(2)+\"%\"),o.style.setProperty(\"--fade\",(2.4+p*11).toFixed(2)+\"%\");const h=g(O[n]*it*.3,0,9);e.style.filter=h>.3?`url(#mb-${n})`:\"none\";const r=document.getElementById(`fe-${n}`);if(r&&r.setAttribute(\"stdDeviation\",`0 ${h.toFixed(2)}`),n===\"tens\"){const f=k(.55,.78,t);o.style.opacity=f,o.style.width=(.6*f).toFixed(4)+\"em\"}}}let G=24;const C=()=>__eviiveVP.w/__eviiveVP.h<1.15||__eviiveVP.w<900;function Nt(){const t=v.getBoundingClientRect();G=Math.round(C()?t.left:__eviiveVP.w-t.right)}const Xt=22,wt=3,at=80,Ut=30,zt=34,Kt=10,Zt=17,jt=32,Jt=18,Qt=10,Ae=[6,54],v=document.getElementById(\"rail\"),lt=[];for(let t=0;t<at;t++){const n=document.createElement(\"i\");v.appendChild(n),lt.push(n)}const bt=[{m:6,label:\"not aligned with prediction\"},{m:54,label:\"aligned with prediction\"}],Et=bt.map(t=>{const n=document.createElement(\"i\");return n.className=\"mark\",n.innerHTML=`<b></b><span>${t.label}</span>`,v.appendChild(n),n}),K=document.getElementById(\"odounit\"),A=document.querySelector(\".readout\"),u={top:0,h:1,left:0,w:1},Z=[],j={w:-1,h:-1},Mt=document.querySelector(\".readout .unit\");function Ct(t){const n=t.style.whiteSpace;t.style.whiteSpace=\"nowrap\";const e=document.createElement(\"span\");e.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",t.appendChild(e);const o=e.getBoundingClientRect().bottom-t.getBoundingClientRect().top;return e.remove(),t.style.whiteSpace=n,o}function Rt(t){const n=getComputedStyle(t),e=document.createElement(\"span\");e.style.cssText=`position:absolute;left:-9999px;top:0;visibility:hidden;display:block;white-space:nowrap;font-family:${n.fontFamily};font-size:${n.fontSize};font-weight:${n.fontWeight};letter-spacing:${n.letterSpacing};line-height:${n.lineHeight}`,e.textContent=\"x\",document.body.appendChild(e);const o=document.createElement(\"span\");o.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",e.appendChild(o);const i=e.getBoundingClientRect().bottom-o.getBoundingClientRect().bottom;return e.remove(),i}function J(){j.w=__eviiveVP.w,j.h=__eviiveVP.h,a.stacked=C();var t=document.querySelector(\".eviive-root\")||document.documentElement;t.classList.toggle(\"stacked\",a.stacked),Nt(),m.style.left=m.style.right=m.style.bottom=\"\",m.style.textAlign=m.style.maxWidth=\"\",A.style.top=A.style.bottom=\"\",v.style.top=v.style.bottom=\"\";const n=parseFloat(getComputedStyle(document.querySelector(\".eviive-root\")||document.documentElement).getPropertyValue(\"--nav-h\"))||0;a.orbY=.5+n/(2*__eviiveVP.h),a.orbX=.5,a.orbDia=0;const e=z.querySelectorAll(\".odo-col\"),o=e[e.length-1],i=o&&o.querySelector(\"i\");if(!i)return;const l=parseFloat(getComputedStyle(z).fontSize);K.style.transform=\"none\";const s=o.getBoundingClientRect().top+.2744*l+Ct(i),p=K.getBoundingClientRect().top+Ct(K);K.style.transform=`translateY(${(s-p).toFixed(2)}px)`;const h=v.getBoundingClientRect();if(u.top=h.top,u.h=h.height,u.left=h.left,u.w=h.width,C()){m.style.transform=\"none\",A.style.transform=\"none\";const L=parseFloat(getComputedStyle(document.documentElement).getPropertyValue(\"--pad-b\"))||96,_t=40,gt=32,ve=m.getBoundingClientRect().height,Lt=__eviiveVP.w>809;m.style.bottom=L+\"px\",Lt&&(m.style.left=\"auto\",m.style.right=G+\"px\",m.style.textAlign=\"right\",m.style.maxWidth=\"44%\");const kt=Lt?L:L+ve+_t;A.style.top=\"auto\",A.style.bottom=kt.toFixed(0)+\"px\";const xe=A.getBoundingClientRect().height;v.style.top=\"auto\",v.style.bottom=(kt+xe+gt).toFixed(0)+\"px\";const U=v.getBoundingClientRect();u.top=U.top,u.h=U.height,u.left=U.left,u.w=U.width;const we=40,be=0,Ee=.84,Me=document.querySelector(\".headline\"),Ce=[q,W,H].map(vt=>vt.style.display);q.style.display=\"none\",W.style.display=\"\",H.style.display=\"\";const Re=Me.getBoundingClientRect().bottom;[q,W,H].forEach((vt,Se)=>{vt.style.display=Ce[Se]});const Vt=Re+be,$t=U.top-we;a.orbY=(Vt+$t)/2/__eviiveVP.h;const Pe=__eviiveVP.w-2*G;a.orbDia=Math.max(Math.min(($t-Vt)*Ee,Pe*.78),0);return}const r=v.getBoundingClientRect().bottom;m.style.transform=\"none\";const f=m.getBoundingClientRect().bottom-Rt(m);m.style.transform=`translateY(${(r-f).toFixed(2)}px)`,A.style.transform=\"none\";const V=Mt.getBoundingClientRect().bottom-Rt(Mt);A.style.transform=`translateY(${(r-V).toFixed(2)}px)`,Et.forEach((L,_t)=>{const gt=L.style.display;L.style.display=\"block\",Z[_t]=L.lastElementChild.offsetWidth,L.style.display=gt});const R=document.querySelector(\".headline\"),w={right:R.offsetLeft+R.offsetWidth,bottom:R.offsetTop+R.offsetHeight},B=__eviiveVP.w-G-30,c=Math.max(Z[0]||0,Z[1]||0)||200,y=w.right+24,_=B-c-40,b=__eviiveVP.h/2/Math.tan(38*Math.PI/360)/6.4,P=__eviiveVP.w/2,E=(a.orbY||.5)*__eviiveVP.h,F=Math.max(0,E-w.bottom),S=P-(w.right-4),$=S>=0?Math.sqrt(S*S+F*F):F,T=B-c-P,Ft=Math.min($,T);a._dbg={A:Math.round(S),dy:Math.round(F),orbCy:Math.round(E),hlB:Math.round(w.bottom),hlR:Math.round(w.right),capL:Math.round($),capR:Math.round(T),natR:Math.round(b),orbY:+(a.orbY||0).toFixed(4)},a.orbX=.5,a.orbDia=Ft>=b?0:Math.max(Ft*2,80)}function te(t){u.top=v.offsetTop,u.h=v.offsetHeight,u.left=v.offsetLeft,u.w=v.offsetWidth;const n=C()?u.w:u.h,e=C()?__eviiveVP.w*.5-u.left:(a.orbY||.5)*__eviiveVP.h-u.top,i=Math.max(e,1)/Math.max(t,.05),l=a.p,s=__eviiveVP.h/2/Math.tan(38*Math.PI/360),p=__eviiveVP.w*(a.orbX||.5)+s/6.4,h=__eviiveVP.w-G-30,r=k(.56,.86,l),f=[1-r,r],V=[];bt.forEach((c,y)=>{const _=e+(c.m-t)*i,d=Et[y],b=d.lastElementChild,P=Math.min(k(-60,20,_),1-k(n*.56,n*.78,_)),E=f[y]*P;if(d.style.display=E>.004?\"block\":\"none\",E<=.004)return;V.push({m:c.m,o:E});const F=Z[y]||1,$=30+Math.max((h-p-F)/2,40)*(.52+.48*Q(_,n)),T=C()?jt+Jt*Q(_,n):Math.max($-Xt,24);C()?(d.style.transform=`translate3d(${_.toFixed(1)}px,0,0)`,d.firstElementChild.style.transform=`scaleY(${(T/220).toFixed(4)})`,b.style.transform=`translate3d(-50%,${-(T+Qt).toFixed(1)}px,0)`):(d.style.transform=`translate3d(0,${_.toFixed(1)}px,0)`,d.firstElementChild.style.transform=`scaleX(${(T/300).toFixed(4)})`,b.style.transform=`translate3d(${(-$).toFixed(1)}px,-50%,0)`),d.style.opacity=E.toFixed(3),b.style.filter=E>.995?\"none\":`blur(${((1-E)*20).toFixed(2)}px)`});const R=Math.min(Math.abs(l),Math.abs(l-.5),Math.abs(l-1))<.004;let w=0,B=0;for(;B<at;w+=wt){const c=e+(w-t)*i;if(c>n+12)break;if(c<-12)continue;const y=V.find($=>Math.abs($.m-w)<wt*.5),_=y?y.o:0;if(_>.996)continue;const d=lt[B++];d._on!==1&&(d.style.display=\"block\",d._on=1);const b=Math.round(c*10)/10,P=Math.round(C()?Kt+Zt*Q(c,n):Ut+zt*Q(c,n));(d._y!==b||d._w!==P)&&(d.style.transform=C()?`translate3d(${b}px,0,0) scaleY(${(P/64).toFixed(4)})`:`translate3d(0,${b}px,0) scaleX(${(P/64).toFixed(4)})`,d._y=b,d._w=P);const F=(R?c<-2||c>n+2?0:1:c<0?Math.max(0,1+c/12):c>n?Math.max(0,1-(c-n)/12):1)*(1-_),S=Math.round(F*50)/50;d._e!==S&&(d.style.opacity=S,d._e=S)}for(let c=B;c<at;c++){const y=lt[c];y._on!==0&&(y.style.display=\"none\",y._on=0)}}const ee=1.9;function Q(t,n){const e=Math.min(Math.abs(t/n-.5)*2,1),o=Math.cos(e*Math.PI/2);return Math.pow(o*o,ee)}function ne(t){const n=t.textContent.split(/(\\s+)/);t.textContent=\"\";const e=[];for(const o of n){if(!o.trim()&&o.includes(`\n`)){t.appendChild(document.createElement(\"br\"));continue}const i=document.createElement(\"span\");i.className=\"wd\";const l=document.createElement(\"b\");l.textContent=o,i.appendChild(l),t.appendChild(i),o.trim()&&e.push(l)}return e}const oe=new Map([q,W,H].map(t=>[t,ne(t)])),tt=850,rt=100,se=240,ie=10,Pt=10,dt=St(.44,0,.56,1),pt=[{els:[q],inAt:()=>x<=.5,armAt:()=>x===.5,armExact:!0,exitY:-1,canReverse:!0},{els:[W,H],inAt:()=>x>=1,exitY:-1,exitReverse:!0}],mt=380,ut=.45;for(const t of pt)t.words=t.els.reduce((n,e)=>n.concat(oe.get(e)),[]),t.span=tt+rt*Math.max(t.words.length-1,0),t.maxD0=rt*Math.max(t.words.length-1,0),t.xspan=mt+t.maxD0*ut,t.t=0,t.x=0;function et(){for(const t of pt){const n=t.els.map(l=>l.style.display);t.els.forEach(l=>{l.style.display=\"\"});let e=null,o=-1,i=0;for(const l of t.words){const s=Math.round(l.getBoundingClientRect().top);(e===null||Math.abs(s-e)>4)&&(o++,e=s,i=0),l._d0=o*se+i*rt,i++}t.maxD0=Math.max(0,...t.words.map(l=>l._d0)),t.span=tt+t.maxD0,t.xspan=mt+t.maxD0*ut,t.els.forEach((l,s)=>{l.style.display=n[s]})}}let ht=-1,ft=0,N=16,nt=1,yt=0;function ce(t,n){N=ht<0?16:Math.min(n-ht,64),ht=n,x!==yt&&(nt=x>yt?1:-1,yt=x);for(const e of pt){const o=e.inAt(),i=e.armAt?e.armAt():o,l=e.x>.02;o&&!e.was&&(e.x=0),i&&!e.wasArmed&&(e.revEntry=l,e.t=0,e.x=0),e.was=o,e.wasArmed=i,o?(e.x=g(e.x-N/e.xspan,0,1),i&&(e.t=g(e.t+N/e.span,0,1))):(e.x=g(e.x+N/e.xspan,0,1),e.x>=1&&(e.t=0));for(const r of e.els)r._clr!==1&&(r.style.transform=\"none\",r.style.opacity=\"\",r.style.filter=\"none\",r._clr=1);const s=e.x>1e-4,p=s?e.x<.999:e.t>5e-4;for(const r of e.els){const f=p?\"\":\"none\";r._d!==f&&(r.style.display=f,r._d=f)}if(!p)continue;const h=s?e.x*e.xspan:e.t*e.span;for(let r=0;r<e.words.length;r++){const f=e.words[r],V=f._d0||0,R=nt<0?e.maxD0-V:V,w=s?1-dt(g((h-R*ut)/mt,0,1)):dt(g((h-R)/tt,0,1)),B=s?-nt:nt,c=f,y=Math.round(w*100)/100;c._o!==y&&(c.style.opacity=y,c._o=y);const _=Math.round((1-w)*ie*B*4)/4;c._y!==_&&(c.style.transform=Math.abs(_)<.2?\"none\":`translate3d(0,${_}px,0)`,c._y=_);const d=Math.round((1-w)*Pt*4)/4;c._b!==d&&(c.style.filter=d<.2?\"none\":`blur(${d}px)`,c._b=d)}}}const ot=(t,n,e)=>`rgba(${Math.round(M(t[0],n[0],e))},${Math.round(M(t[1],n[1],e))},${Math.round(M(t[2],n[2],e))},${M(t[3],n[3],e).toFixed(3)})`,ae=[255,255,255,.96],le=[11,32,40,.94],re=[248,246,242,.42],de=[11,32,40,.52],pe=[248,246,242,.22],me=[11,32,40,.24],ue=[103,234,255,1],he=[6,132,177,1];function fe(t){D.style.setProperty(\"--ink\",ot(ae,le,t)),D.style.setProperty(\"--soft\",ot(re,de,t)),D.style.setProperty(\"--ghost\",ot(pe,me,t)),D.style.setProperty(\"--accent\",ot(ue,he,t))}function St(t,n,e,o){const i=s=>{const p=1-s;return 3*p*p*s*t+3*p*s*s*e+s*s*s},l=s=>{const p=1-s;return 3*p*p*s*n+3*p*s*s*o+s*s*s};return s=>{if(s<=0)return 0;if(s>=1)return 1;let p=0,h=1,r=0;for(let f=0;f<24;f++)r=(p+h)/2,i(r)<s?p=r:h=r;return l((p+h)/2)}}const ye=St(.37,0,.63,1),_e=2600;let x=0,I=0,X=-1e9;const ge=.75;function At(){if(window.__eviiveDriving||window.__eviiveOwned)return;const t=Ht.getBoundingClientRect(),n=t.height-__eviiveVP.h;if(t.top>0){const i=g(1-t.top/__eviiveVP.h,0,1);x=I=a.p=g(i/ge,0,1)*.5,X=-1e9;return}const e=n>0?g(-t.top/n,0,1):0,o=.5+Math.round(e)/2;o!==x&&(I=a.p,x=o,X=performance.now())}J(),et(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(()=>{J(),et()}),addEventListener(\"resize\",()=>{J(),et()}),addEventListener(\"scroll\",At,{passive:!0}),At();function Bt(t){if(!__eviiveDead){requestAnimationFrame(Bt);try{window.__eviiveTick=(window.__eviiveTick|0)+1,a.p=It?x:I+(x-I)*ye(g((t-X)/_e,0,1));const n=a.p;try{(j.w!==__eviiveVP.w||j.h!==__eviiveVP.h)&&(J(),et()),Gt(n),te(n<=.5?M(0,6,n/.5):Math.exp(M(Math.log(6),Math.log(54),(n-.5)/.5))),ce(n,t),fe(k(.58,.96,n))}catch(o){window.__eviiveErr||(window.__eviiveErr=o.stack||String(o),console.error(\"[eviive] frame:\",o))}D.style.opacity=k(.02,.14,n),ft=g(ft+(x>=1?1:-1)*N/tt,0,1);const e=dt(ft);m.style.opacity=(e*.9).toFixed(3),m.style.filter=e>.995?\"none\":`blur(${((1-e)*Pt).toFixed(2)}px)`,qt.style.opacity=1-k(.02,.18,n),xt.classList.contains(\"on\")&&(xt.textContent=`p       ${n.toFixed(3)}\ncamZ    ${(-47.1*n).toFixed(2)}\nmonths  ${M(Tt,Dt,n).toFixed(1)}`)}catch(n){window.__eviiveErr||(window.__eviiveErr=n.stack||String(n),console.error(\"[eviive] frame:\",n))}}}requestAnimationFrame(Bt),window.__eviive={to(t){return I=a.p,x=t,X=performance.now(),t},seek(t){return x=I=a.p=g(t,0,1),X=-1e9,a.p},get progress(){return a.p},states:{a:0,b:1}}})();\n"
...
export default function mount(host) {
  ...
  const initT = setInterval(() => {
    ...
    if (zone() === "inside") {
      ...
      engaged = true
      a.seek(0.5)
    } else if (!engaged && !spent) a.seek(0)
    updateHint()
  }, 150)
