...
const DRIVER = "window.__eviiveStop&&window.__eviiveStop();let __eviiveDead=!1;window.__eviiveStop=function(){__eviiveDead=!0},(()=>{\"use strict\";const Nt=matchMedia(\"(prefers-reduced-motion: reduce)\").matches,g=(t,n,e)=>t<n?n:t>e?e:t,M=(t,n,e)=>t+(n-t)*e,k=(t,n,e)=>{const o=g((e-t)/(n-t),0,1);return o*o*(3-2*o)},Xt=6.4,Ut=53.5,c=window.__eviiveScroll=window.__eviiveScroll||{p:0};c.mx=c.my=0,addEventListener(\"pointermove\",t=>{c.mx=t.clientX/__eviiveVP.w*2-1,c.my=1-t.clientY/__eviiveVP.h*2},{passive:!0});const zt=document.getElementById(\"pin\"),W=document.getElementById(\"ui\"),$=document.getElementById(\"line2\"),Y=document.getElementById(\"l1a\"),F=document.getElementById(\"l1b\"),m=document.getElementById(\"caption\"),Zt=document.getElementById(\"cue\"),Mt=document.getElementById(\"hud\"),Z=document.getElementById(\"odo\"),jt=[{key:\"tens\",mul:.1},{key:\"ones\",mul:1},{key:\"point\"},{key:\"tenths\",mul:10}],O={},ct={},G={};let at=102;for(const t of jt){if(t.key===\"point\"){const o=document.createElement(\"div\");o.className=\"odo-pt\",o.textContent=\".\",Z.appendChild(o);continue}const n=document.createElement(\"div\");n.className=\"odo-col\";const e=document.createElement(\"div\");e.className=\"stack\";for(let o=0;o<=44;o++){const s=document.createElement(\"i\");s.textContent=String(o%10),e.appendChild(s)}n.appendChild(e),Z.appendChild(n),O[t.key]={st:e,col:n,mul:t.mul},G[t.key]=0}function rt(){O.ones&&(at=O.ones.col.getBoundingClientRect().height||at)}rt(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(rt),addEventListener(\"resize\",rt,{passive:!0});const Jt={tens:[0,0,5],ones:[0,6,23],tenths:[0,14,35]};function Qt(t,n){const e=Jt[t];return n<=.5?M(e[0],e[1],n/.5):M(e[1],e[2],(n-.5)/.5)}function te(t){for(const n in O){const{st:e,col:o}=O[n],s=Qt(n,t);e.style.transform=`translateY(${(.275-s).toFixed(4)}em)`;const r=ct[n]===void 0?s:ct[n];ct[n]=s;const i=Math.abs(s-r);G[n]=G[n]*.84+i*.16;const p=g(G[n]*30,0,1);o.style.setProperty(\"--eat\",(p*11.5).toFixed(2)+\"%\"),o.style.setProperty(\"--fade\",(2.4+p*11).toFixed(2)+\"%\");const u=g(G[n]*at*.3,0,9);e.style.filter=u>.3?`url(#mb-${n})`:\"none\";const l=document.getElementById(`fe-${n}`);if(l&&l.setAttribute(\"stdDeviation\",`0 ${u.toFixed(2)}`),n===\"tens\"){const y=k(.55,.78,t);o.style.opacity=y,o.style.width=(.6*y).toFixed(4)+\"em\"}}}let K=24;const b=()=>__eviiveVP.w/__eviiveVP.h<1.15||__eviiveVP.w<900;function ee(){const t=v.getBoundingClientRect();K=Math.round(b()?t.left:__eviiveVP.w-t.right)}const Pt=22,Rt=3,lt=80,dt=30,pt=34,Ct=10,At=17,Fe=32,ne=18,oe=10,Ve=[6,54],v=document.getElementById(\"rail\"),mt=[];for(let t=0;t<lt;t++){const n=document.createElement(\"i\");v.appendChild(n),mt.push(n)}const St=[{m:6,label:\"not aligned with prediction\"},{m:54,label:\"aligned with prediction\"}],Bt=St.map(t=>{const n=document.createElement(\"i\");return n.className=\"mark\",n.innerHTML=`<b></b><span>${t.label}</span>`,v.appendChild(n),n}),j=document.getElementById(\"odounit\"),C=document.querySelector(\".readout\"),f={top:0,h:1,left:0,w:1},J=[],Q={w:-1,h:-1},Lt=document.querySelector(\".readout .unit\");function kt(t){const n=t.style.whiteSpace;t.style.whiteSpace=\"nowrap\";const e=document.createElement(\"span\");e.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",t.appendChild(e);const o=e.getBoundingClientRect().bottom-t.getBoundingClientRect().top;return e.remove(),t.style.whiteSpace=n,o}function Ft(t){const n=getComputedStyle(t),e=document.createElement(\"span\");e.style.cssText=`position:absolute;left:-9999px;top:0;visibility:hidden;display:block;white-space:nowrap;font-family:${n.fontFamily};font-size:${n.fontSize};font-weight:${n.fontWeight};letter-spacing:${n.letterSpacing};line-height:${n.lineHeight}`,e.textContent=\"x\",document.body.appendChild(e);const o=document.createElement(\"span\");o.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",e.appendChild(o);const s=e.getBoundingClientRect().bottom-o.getBoundingClientRect().bottom;return e.remove(),s}const Vt=document.querySelector(\".sticky\");function It(){const t=[...F.querySelectorAll(\".wd\"),...$.querySelectorAll(\".wd\")];if(!t.length)return;if(!b()){t.forEach(e=>e.classList.remove(\"pI\",\"pA\"));return}const n=new Map;t.forEach(e=>{const o=Math.round(e.offsetTop/8);n.has(o)||n.set(o,[]),n.get(o).push(e)}),n.forEach(e=>{const o=e.some(s=>s.parentElement===F);e.forEach(s=>{s.classList.toggle(\"pI\",o),s.classList.toggle(\"pA\",!o)})})}function tt(){if(__eviiveVP.h<100)return;Q.w=__eviiveVP.w,Q.h=__eviiveVP.h,c.stacked=b();var t=document.querySelector(\".eviive-root\")||document.documentElement;t.classList.toggle(\"stacked\",c.stacked),ee(),m.style.left=m.style.right=m.style.bottom=\"\",m.style.textAlign=m.style.maxWidth=\"\",C.style.top=C.style.bottom=\"\",v.style.top=v.style.bottom=\"\";const n=parseFloat(getComputedStyle(document.querySelector(\".eviive-root\")||document.documentElement).getPropertyValue(\"--nav-h\"))||0;c.orbY=.5+n/(2*__eviiveVP.h),c.orbX=.5,c.orbDia=0;const e=Z.querySelectorAll(\".odo-col\"),o=e[e.length-1],s=o&&o.querySelector(\"i\");if(!s)return;const r=parseFloat(getComputedStyle(Z).fontSize);j.style.transform=\"none\";const i=o.getBoundingClientRect().top+.2744*r+kt(s),p=j.getBoundingClientRect().top+kt(j);j.style.transform=`translateY(${(i-p).toFixed(2)}px)`;const u=v.getBoundingClientRect();if(f.top=u.top,f.h=u.height,f.left=u.left,f.w=u.width,b()){m.style.transform=\"none\",C.style.transform=\"none\";const L=parseFloat(getComputedStyle(document.documentElement).getPropertyValue(\"--pad-b\"))||96,wt=40,bt=32,Ee=m.getBoundingClientRect().height,Wt=__eviiveVP.w>809;m.style.bottom=L+\"px\",Wt&&(m.style.left=\"auto\",m.style.right=K+\"px\",m.style.textAlign=\"right\",m.style.maxWidth=\"44%\");const Yt=Wt?L:L+Ee+wt;C.style.top=\"auto\",C.style.bottom=Yt.toFixed(0)+\"px\";const Me=C.getBoundingClientRect().height;v.style.top=\"auto\",v.style.bottom=(Yt+Me+bt).toFixed(0)+\"px\";const z=v.getBoundingClientRect();f.top=z.top,f.h=z.height,f.left=z.left,f.w=z.width;const Pe=40,Re=0,Ce=.84,Ae=document.querySelector(\".headline\"),Se=[Y,F,$].map(Et=>Et.style.display);Y.style.display=\"none\",F.style.display=\"\",$.style.display=\"\";const Be=Ae.getBoundingClientRect().bottom;It(),[Y,F,$].forEach((Et,ke)=>{Et.style.display=Se[ke]});const Ot=Vt?Vt.getBoundingClientRect().top:0,Gt=Be-Ot+Re,Kt=z.top-Ot-Pe;c.orbY=(Gt+Kt)/2/__eviiveVP.h;const Le=__eviiveVP.w-2*K;c.orbDia=Math.max(Math.min((Kt-Gt)*Ce,Le*.78,__eviiveVP.w*.34),0);return}const l=v.getBoundingClientRect().bottom;m.style.transform=\"none\";const y=m.getBoundingClientRect().bottom-Ft(m);m.style.transform=`translateY(${(l-y).toFixed(2)}px)`,C.style.transform=\"none\";const V=Lt.getBoundingClientRect().bottom-Ft(Lt);C.style.transform=`translateY(${(l-V).toFixed(2)}px)`,Bt.forEach((L,wt)=>{const bt=L.style.display;L.style.display=\"block\",J[wt]=L.lastElementChild.offsetWidth,L.style.display=bt});const P=document.querySelector(\".headline\"),w={right:P.offsetLeft+P.offsetWidth,bottom:P.offsetTop+P.offsetHeight},A=__eviiveVP.w-K-30,a=Math.max(J[0]||0,J[1]||0)||200;It();const h=Math.max(.55,Math.min(1,(__eviiveVP.w-900)/612));c.railK=h;const _=Math.round((dt+pt)*h);c.reachMin=_+Pt+8;const d=w.right+24,R=A-a-40,E=__eviiveVP.h/2/Math.tan(38*Math.PI/360)/6.4,D=__eviiveVP.w/2,S=(c.orbY||.5)*__eviiveVP.h,B=Math.max(0,S-w.bottom),H=D-(w.right-4),U=H>=0?Math.sqrt(H*H+B*B):B,q=A-c.reachMin-a-16-D,qt=Math.min(U,q);c._dbg={A:Math.round(H),dy:Math.round(B),orbCy:Math.round(S),hlB:Math.round(w.bottom),hlR:Math.round(w.right),capL:Math.round(U),capR:Math.round(q),natR:Math.round(E),orbY:+(c.orbY||0).toFixed(4)},c.orbX=.5,c.orbDia=qt>=E?0:Math.max(qt*2,80)}function se(t){f.top=v.offsetTop,f.h=v.offsetHeight,f.left=v.offsetLeft,f.w=v.offsetWidth;const n=b()?f.w:f.h,e=b()?__eviiveVP.w*.5-f.left:(c.orbY||.5)*__eviiveVP.h-f.top,s=Math.max(e,1)/Math.max(t,.05),r=c.p,i=__eviiveVP.h/2/Math.tan(38*Math.PI/360),p=__eviiveVP.w-K-30,u=(dt+pt)*(c.railK||1),l=k(.56,.86,r),y=[1-l,l],V=[];St.forEach((a,h)=>{const _=e+(a.m-t)*s,d=Bt[h],R=d.lastElementChild,I=Math.min(k(-60,20,_),1-k(n*.56,n*.78,_)),E=y[h]*I;if(d.style.display=E>.004?\"block\":\"none\",E<=.004)return;V.push({m:a.m,o:E});const D=J[h]||1,S=__eviiveVP.w*(c.orbX||.5)+((h===0?c.orbPxRestA:c.orbPxRestB)||i/6.4),B=p-u-S,H=Math.max((B-D)/2,24),U=u+H*(.8+.2*et(_,n)),q=b()?Ct+At+10+ne*et(_,n):Math.max(U-Pt,u+6);b()?(d.style.transform=`translate3d(${_.toFixed(1)}px,0,0)`,d.firstElementChild.style.transform=`scaleY(${(q/220).toFixed(4)})`,R.style.transform=`translate3d(-50%,${-(q+oe).toFixed(1)}px,0)`):(d.style.transform=`translate3d(0,${_.toFixed(1)}px,0)`,d.firstElementChild.style.transform=`scaleX(${(q/300).toFixed(4)})`,R.style.transform=`translate3d(${(-U).toFixed(1)}px,-50%,0)`),d.style.opacity=E.toFixed(3),R.style.filter=E>.995?\"none\":`blur(${((1-E)*20).toFixed(2)}px)`});const P=Math.min(Math.abs(r),Math.abs(r-.5),Math.abs(r-1))<.004;let w=0,A=0;for(;A<lt;w+=Rt){const a=e+(w-t)*s;if(a>n+12)break;if(a<-12)continue;const h=V.find(B=>Math.abs(B.m-w)<Rt*.5),_=h?h.o:0;if(_>.996)continue;const d=mt[A++];d._on!==1&&(d.style.display=\"block\",d._on=1);const R=Math.round(a*10)/10,I=Math.round(b()?Ct+At*et(a,n):(dt+pt*et(a,n))*(c.railK||1));(d._y!==R||d._w!==I)&&(d.style.transform=b()?`translate3d(${R}px,0,0) scaleY(${(I/64).toFixed(4)})`:`translate3d(0,${R}px,0) scaleX(${(I/64).toFixed(4)})`,d._y=R,d._w=I);const D=(P?a<-2||a>n+2?0:1:a<0?Math.max(0,1+a/12):a>n?Math.max(0,1-(a-n)/12):1)*(1-_),S=Math.round(D*50)/50;d._e!==S&&(d.style.opacity=S,d._e=S)}for(let a=A;a<lt;a++){const h=mt[a];h._on!==0&&(h.style.display=\"none\",h._on=0)}}const ie=1.9;function et(t,n){const e=Math.min(Math.abs(t/n-.5)*2,1),o=Math.cos(e*Math.PI/2);return Math.pow(o*o,ie)}function ce(t){const n=t.textContent.split(/(\\s+)/);t.textContent=\"\";const e=[];for(const o of n){if(!o.trim()&&o.includes(`\n`)){t.appendChild(document.createElement(\"br\"));continue}const s=document.createElement(\"span\");s.className=\"wd\";const r=document.createElement(\"b\");r.textContent=o,s.appendChild(r),t.appendChild(s),o.trim()&&e.push(r)}return e}const ae=new Map([Y,F,$].map(t=>[t,ce(t)])),nt=850,ut=100,re=240,le=10,$t=10,ht=Tt(.44,0,.56,1),ft=[{els:[Y],inAt:()=>x<=.5,armAt:()=>x===.5,armExact:!0,exitY:-1,canReverse:!0},{els:[F,$],inAt:()=>x>=1,exitY:-1,exitReverse:!0}],yt=380,_t=.45;for(const t of ft)t.words=t.els.reduce((n,e)=>n.concat(ae.get(e)),[]),t.span=nt+ut*Math.max(t.words.length-1,0),t.maxD0=ut*Math.max(t.words.length-1,0),t.xspan=yt+t.maxD0*_t,t.t=0,t.x=0;function ot(){for(const t of ft){const n=t.els.map(r=>r.style.display);t.els.forEach(r=>{r.style.display=\"\"});let e=null,o=-1,s=0;for(const r of t.words){const i=Math.round(r.getBoundingClientRect().top);(e===null||Math.abs(i-e)>4)&&(o++,e=i,s=0),r._d0=o*re+s*ut,s++}t.maxD0=Math.max(0,...t.words.map(r=>r._d0)),t.span=nt+t.maxD0,t.xspan=yt+t.maxD0*_t,t.els.forEach((r,i)=>{r.style.display=n[i]})}}let gt=-1,vt=0,N=16,st=1,xt=0;function de(t,n){N=gt<0?16:Math.min(n-gt,64),gt=n,x!==xt&&(st=x>xt?1:-1,xt=x);for(const e of ft){const o=e.inAt(),s=e.armAt?e.armAt():o,r=e.x>.02;o&&!e.was&&(e.x=0),s&&!e.wasArmed&&(e.revEntry=r,e.t=0,e.x=0),e.was=o,e.wasArmed=s,o?(e.x=g(e.x-N/e.xspan,0,1),s&&(e.t=g(e.t+N/e.span,0,1))):(e.x=g(e.x+N/e.xspan,0,1),e.x>=1&&(e.t=0));for(const l of e.els)l._clr!==1&&(l.style.transform=\"none\",l.style.opacity=\"\",l.style.filter=\"none\",l._clr=1);const i=e.x>1e-4,p=i?e.x<.999:e.t>5e-4;for(const l of e.els){const y=p?\"\":\"none\";l._d!==y&&(l.style.display=y,l._d=y)}if(!p)continue;const u=i?e.x*e.xspan:e.t*e.span;for(let l=0;l<e.words.length;l++){const y=e.words[l],V=y._d0||0,P=st<0?e.maxD0-V:V,w=i?1-ht(g((u-P*_t)/yt,0,1)):ht(g((u-P)/nt,0,1)),A=i?-st:st,a=y,h=Math.round(w*100)/100;a._o!==h&&(a.style.opacity=h,a._o=h);const _=Math.round((1-w)*le*A*4)/4;a._y!==_&&(a.style.transform=Math.abs(_)<.2?\"none\":`translate3d(0,${_}px,0)`,a._y=_);const d=Math.round((1-w)*$t*4)/4;a._b!==d&&(a.style.filter=d<.2?\"none\":`blur(${d}px)`,a._b=d)}}}const it=(t,n,e)=>`rgba(${Math.round(M(t[0],n[0],e))},${Math.round(M(t[1],n[1],e))},${Math.round(M(t[2],n[2],e))},${M(t[3],n[3],e).toFixed(3)})`,pe=[255,255,255,.96],me=[11,32,40,.94],ue=[248,246,242,.42],he=[11,32,40,.52],fe=[248,246,242,.22],ye=[11,32,40,.24],_e=[103,234,255,1],ge=[6,132,177,1];function ve(t){W.style.setProperty(\"--ink\",it(pe,me,t)),W.style.setProperty(\"--soft\",it(ue,he,t)),W.style.setProperty(\"--ghost\",it(fe,ye,t)),W.style.setProperty(\"--accent\",it(_e,ge,t))}function Tt(t,n,e,o){const s=i=>{const p=1-i;return 3*p*p*i*t+3*p*i*i*e+i*i*i},r=i=>{const p=1-i;return 3*p*p*i*n+3*p*i*i*o+i*i*i};return i=>{if(i<=0)return 0;if(i>=1)return 1;let p=0,u=1,l=0;for(let y=0;y<24;y++)l=(p+u)/2,s(l)<i?p=l:u=l;return r((p+u)/2)}}const xe=Tt(.37,0,.63,1),we=2600;let x=0,T=0,X=-1e9;const be=.75;function Dt(){if(window.__eviiveDriving||window.__eviiveOwned)return;const t=zt.getBoundingClientRect(),n=t.height-__eviiveVP.h;if(t.top>0){const s=g(1-t.top/__eviiveVP.h,0,1);x=T=c.p=g(s/be,0,1)*.5,X=-1e9;return}const e=n>0?g(-t.top/n,0,1):0,o=.5+Math.round(e)/2;o!==x&&(T=c.p,x=o,X=performance.now())}tt(),ot(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(()=>{tt(),ot()}),addEventListener(\"resize\",()=>{tt(),ot()}),addEventListener(\"scroll\",Dt,{passive:!0}),Dt();function Ht(t){if(!__eviiveDead){requestAnimationFrame(Ht);try{window.__eviiveTick=(window.__eviiveTick|0)+1,c.p=Nt?x:T+(x-T)*xe(g((t-X)/we,0,1));const n=c.p;try{(Q.w!==__eviiveVP.w||Q.h!==__eviiveVP.h)&&(tt(),ot()),te(n),se(n<=.5?M(0,6,n/.5):Math.exp(M(Math.log(6),Math.log(54),(n-.5)/.5))),de(n,t),ve(k(.58,.96,n))}catch(o){window.__eviiveErr||(window.__eviiveErr=o.stack||String(o),console.error(\"[eviive] frame:\",o))}W.style.opacity=k(.02,.14,n),vt=g(vt+(x>=1?1:-1)*N/nt,0,1);const e=ht(vt);m.style.opacity=(e*.9).toFixed(3),m.style.filter=e>.995?\"none\":`blur(${((1-e)*$t).toFixed(2)}px)`,Zt.style.opacity=1-k(.02,.18,n),Mt.classList.contains(\"on\")&&(Mt.textContent=`p       ${n.toFixed(3)}\ncamZ    ${(-47.1*n).toFixed(2)}\nmonths  ${M(Xt,Ut,n).toFixed(1)}`)}catch(n){window.__eviiveErr||(window.__eviiveErr=n.stack||String(n),console.error(\"[eviive] frame:\",n))}}}requestAnimationFrame(Ht),window.__eviive={to(t){return T=c.p,x=t,X=performance.now(),t},seek(t){return x=T=c.p=g(t,0,1),X=-1e9,c.p},get progress(){return c.p},states:{a:0,b:1}}})();\n"
const SCENE = "import{useRef as h,useMemo as E}from\"https://esm.sh/react@19.2.8\";import{createRoot as S}from\"https://esm.sh/react-dom@19.2.8/client?deps=react@19.2.8,react-dom@19.2.8\";import{Canvas as L,useFrame as f,useThree as R}from\"https://esm.sh/@react-three/fiber@9.7.0?deps=react@19.2.8,react-dom@19.2.8,three@0.185.1\";import{EffectComposer as F,DepthOfField as I,Bloom as V,Noise as H,Vignette as G}from\"https://esm.sh/@react-three/postprocessing@3.0.4?deps=react@19.2.8,react-dom@19.2.8,three@0.185.1,postprocessing@6.39.4\";import{BlendFunction as W}from\"https://esm.sh/postprocessing@6.39.4?deps=three@0.185.1\";import*as m from\"https://esm.sh/three@0.185.1\";import{Fragment as ae,jsx as i,jsxs as w}from\"https://esm.sh/react@19.2.8/jsx-runtime\";var T=-6.4,M=-53.5,q=6.4,k=M+q,b=(e,t,n)=>e<t?t:e>n?n:e,a=(e,t,n)=>e+(t-e)*n,p=(e,t,n)=>{let o=b((n-e)/(t-e),0,1);return o*o*(3-2*o)},s=window.__eviiveScroll=window.__eviiveScroll||{p:0};function Y(){let e=h(),t=h(),n=E(()=>({uLift:{value:0},uT:{value:0}}),[]);return f(({camera:o,clock:c})=>{e.current&&e.current.position.copy(o.position);let r=t.current&&t.current.uniforms;r&&(r.uLift.value=p(.58,.96,s.p),r.uT.value=c.elapsedTime)}),w(\"mesh\",{ref:e,scale:140,renderOrder:-1,children:[i(\"sphereGeometry\",{args:[1,48,32]}),i(\"shaderMaterial\",{ref:t,side:m.BackSide,depthWrite:!1,uniforms:n,vertexShader:`\n          varying vec3 vP;\n          void main(){ vP = position; gl_Position = projectionMatrix * modelViewMatrix * vec4(position,1.0); }\n        `,fragmentShader:`\n          precision highp float;\n          varying vec3 vP;\n          uniform float uLift;\n          uniform float uT;\n          float hash(vec2 p){\n            p = fract(p * vec2(443.897, 441.423));\n            p += dot(p, p + 19.19);\n            return fract((p.x + p.y) * p.x);\n          }\n          vec3 s2l(vec3 c){\n            return mix(c / 12.92, pow((c + 0.055) / 1.055, vec3(2.4)), step(vec3(0.04045), c));\n          }\n          void main(){\n            vec3 d = normalize(vP);\n            vec2 q = d.xy;\n            float t1 = uT * 0.047, t2 = uT * 0.034;\n            vec2 flow = vec2(\n              sin(q.y * 1.70 + t1) + 0.50 * sin(q.y * 3.05 - t2 * 1.7),\n              cos(q.x * 1.45 - t2) + 0.50 * cos(q.x * 2.60 + t1 * 1.3)\n            ) * 0.185;\n            vec2 ctr = vec2(sin(uT * 0.039) * 0.17, cos(uT * 0.029) * 0.13);\n            float rad = clamp(length(q + flow - ctr) * 1.12, 0.0, 1.0);\n            vec3 navy     = s2l(vec3(0.008, 0.173, 0.231));  \n            vec3 navyDeep = s2l(vec3(0.004, 0.110, 0.153));   \n            vec3 dark = mix(navy, navyDeep, smoothstep(0.15, 1.0, rad));\n            vec3 liteC = s2l(vec3(0.980, 0.995, 1.000));      \n            vec3 liteE = s2l(vec3(0.741, 0.869, 0.929));      \n            vec3 lite = mix(liteC, liteE, smoothstep(0.10, 1.0, rad));\n            vec3 c = mix(dark, lite, uLift);\n            c += (hash(gl_FragCoord.xy) - 0.5) * (1.5/255.0);   \n            gl_FragColor = vec4(c, 1.0);\n          }\n        `})]})}var Z=`\n  varying vec3 vN; varying vec3 vV; varying vec3 vP;\n  uniform float uT; uniform float uWobAmp; uniform float uOrganic; uniform float uChurn; uniform vec2 uPtr;\n  float h31(vec3 p){ p=fract(p*0.3183099+vec3(.11,.17,.13)); p*=17.0; return fract(p.x*p.y*p.z*(p.x+p.y+p.z)); }\n  float n3(vec3 x){ vec3 i=floor(x),f=fract(x); f=f*f*f*(f*(f*6.0-15.0)+10.0);\n    return mix(mix(mix(h31(i),h31(i+vec3(1,0,0)),f.x),mix(h31(i+vec3(0,1,0)),h31(i+vec3(1,1,0)),f.x),f.y),\n               mix(mix(h31(i+vec3(0,0,1)),h31(i+vec3(1,0,1)),f.x),mix(h31(i+vec3(0,1,1)),h31(i+vec3(1,1,1)),f.x),f.y),f.z);}\n  float dispAt(vec3 pn){\n    float nz = n3(pn * 1.35 + vec3(uPtr * 0.34, uT * 0.19)) - 0.5;\n    float lobes = sin(pn.y * 1.7 + uT * 0.72 + uPtr.y * 0.85)\n                + sin(pn.x * 1.3 - uT * 0.55 + uPtr.x * 0.85);\n    return nz * 1.6 * uOrganic + lobes * 0.5 * uWobAmp;\n  }\n  void main(){\n    vP = position;\n    vec3 pn = normalize(position);\n    float d0 = dispAt(pn);\n    vec3 pos = pn * (1.0 + d0);\n    vec3 up = abs(pn.y) < 0.99 ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);\n    vec3 t1 = normalize(cross(up, pn));\n    vec3 t2 = cross(pn, t1);\n    const float E = 0.045;\n    vec3 pa = normalize(pn + t1 * E), pb = normalize(pn + t2 * E);\n    vec3 va = pa * (1.0 + dispAt(pa)) - pos;\n    vec3 vb = pb * (1.0 + dispAt(pb)) - pos;\n    vec3 bumped = normalize(cross(va, vb));\n    bumped *= sign(dot(bumped, pn));\n    vN = normalize(normalMatrix * bumped);\n    vec4 mv = modelViewMatrix * vec4(pos, 1.0);\n    vV = -mv.xyz;\n    gl_Position = projectionMatrix * mv;\n  }`,U=`\n  precision highp float;\n  varying vec3 vN; varying vec3 vV; varying vec3 vP;\n  uniform float uT; uniform float uAlpha; uniform vec2 uPtr;\n  float ndv(){ return clamp(dot(normalize(vN), normalize(vV)), 0.0, 1.0); }\n  float fres(float k){ return pow(clamp(1.0 - ndv(), 0.0, 1.0), k); }\n  float band(float inner, float outer){\n    float d = ndv();\n    return smoothstep(outer, inner, d) * smoothstep(0.0, 0.10, d);\n  }\n  float nucleus(){\n    vec3 nn = normalize(vN);\n    vec2 off = vec2(sin(uT * 0.13) * 0.12, cos(uT * 0.105) * 0.09);\n    float d = length(nn.xy - off);\n    float k = 1.0 - smoothstep(0.02, 0.62, d);\n    return k * k * k;                  \n  }\n  const vec3 CORE = vec3(0.62,0.875,0.965);   \n  const vec3 MIDB = vec3(0.42,0.865,0.985);\n  const vec3 EDGE = vec3(0.09,0.42,0.60);\n`,j=U+`\n  uniform float uMorph;\n  void main(){\n    float m = uMorph;\n    vec3  N = normalize(vN), V = normalize(vV);\n    vec2  bgCtr = vec2(sin(uT * 0.039) * 0.17, cos(uT * 0.029) * 0.13);\n    vec3  L = normalize(vec3(bgCtr.x * 3.6 - 0.22, bgCtr.y * 3.6 + 0.76, 0.52));\n    float thick = pow(ndv(), 0.70);\n    float nuc = nucleus();\n    float rimA = dot(N, L) * 0.5 + 0.5;\n    float rimW = smoothstep(0.02, 0.98, rimA);\n    vec3 pDeep  = mix(vec3(0.22,0.26,0.30), EDGE, m);\n    vec3 pMid   = mix(vec3(0.30,0.36,0.41), MIDB, m);\n    vec3 pLight = mix(vec3(0.86,0.91,0.94), CORE, m);\n    float tilt = sin(uT * 0.115) * 0.55 * m;\n    float roll = cos(uT * 0.083) * 0.10 * m;\n    float g = smoothstep(-0.85 + roll, 0.75 + roll, N.y + N.x * tilt);\n    g = mix(fres(2.0), g, m);\n    vec3 base = mix(pMid, pLight, g);\n    base = mix(base, mix(pMid, pDeep, 0.55), (1.0 - thick) * mix(0.06, 0.22, m));\n    vec3  Ni = normalize(N + L * 0.44 + vec3(0.0, -0.15, 0.0));\n    float shell = smoothstep(0.05, 0.95, dot(Ni, L)) * thick;\n    base = mix(base, pLight, shell * 0.30 * m);\n    float bandDrift = 0.10 + sin(uT * 0.094) * 0.035;\n    base = mix(base, pDeep,\n               band(bandDrift, bandDrift + 0.52) * 0.62 * m * mix(0.22, 1.0, 1.0 - rimW));\n    base = mix(base, pDeep * 0.72, nuc * 0.62);\n    vec3  Hv = normalize(L + V);\n    float trim = (1.25 + sin(uT * 0.137) * 0.16) * m;\n    vec3  c = base\n            + vec3(0.80,0.95,1.00) * fres(3.0) * trim * mix(0.62, 1.0, rimW)\n            + vec3(0.86,0.96,1.00) * pow(max(dot(N, Hv), 0.0),  5.0) * 0.14 * thick * m\n            + vec3(1.00,1.00,1.00) * pow(max(dot(N, Hv), 0.0), 44.0) * 0.20 * m;\n    float aThin  = mix(0.14, 0.60, fres(1.7));\n    float aGlass = min(1.0, mix(1.0, 0.50, g) + fres(2.2) * 0.60);\n    float a = mix(aThin, aGlass, m) * (1.0 + nuc * mix(0.60, 0.32, m));\n    gl_FragColor = vec4(c, min(a, 1.0) * uAlpha);\n  }`,y=.26,K=1,X=e=>e*e*(3-2*e),J=y/6.4,N=K/6.4,z=14,Q=e=>e<=.5?a(z,0,e/.5):a(0,k,(e-.5)/.5);function C(e){if(e<=.5)return{z:T,r:y,m:0};let t=b((e-.5)/.5,0,1),n=t+.08*Math.sin(Math.PI*t),o=a(T,M,n),c=Math.abs(a(0,k,t)-o);return{z:o,r:c*a(J,N,X(t)),m:p(.1,.82,t)}}function $(){let e=h(),t=h(),n=E(()=>({uT:{value:0},uAlpha:{value:1},uMorph:{value:0},uWobAmp:{value:.012},uOrganic:{value:.016},uChurn:{value:0},uPtr:{value:new m.Vector2}}),[]),o=h({x:0,y:0});return f(({clock:c})=>{let r=t.current&&t.current.uniforms;if(!r||!e.current)return;let l=C(s.p);o.current.x+=((s.mx||0)-o.current.x)*.035,o.current.y+=((s.my||0)-o.current.y)*.035,r.uPtr.value.set(o.current.x,o.current.y),r.uT.value=c.elapsedTime,r.uMorph.value=l.m,r.uWobAmp.value=a(.1,.085,l.m),r.uOrganic.value=a(.095,.105,l.m),r.uChurn.value=0,e.current.position.z=l.z;let d=!!s.stacked,u=Math.tan(38*Math.PI/360),v=__eviiveVP.h/2/u,A=Math.abs(Q(s.p)-l.z),B=(.5-(s.orbY||(d?.355:.5)))*2*u;e.current.position.y=B*A;let D=s.orbX||.5;e.current.position.x=d?0:(D-.5)*2*u*(__eviiveVP.w/__eviiveVP.h)*A;let P=1+Math.sin(c.elapsedTime*.62)*a(.062,.024,l.m),g=s.orbDia?s.orbDia*6.4/(2*v):null,_=2*N*v,O=d?1:Math.min(1,__eviiveVP.w*.34/_),x=d?g!==null?g:.52:Math.min(g!==null?g:1,O);s.orbPxRestA=x*v*(y/6.4),s.orbPxRestB=x*v*(1/6.4),r.uAlpha.value=p(0,.12,s.p),e.current.scale.setScalar(l.r*P*x)}),w(\"mesh\",{ref:e,position:[0,0,T],scale:y,children:[i(\"sphereGeometry\",{args:[1,112,80]}),i(\"shaderMaterial\",{ref:t,uniforms:n,vertexShader:Z,fragmentShader:j,transparent:!0})]})}function ee(){let e=h(),t=E(()=>new m.Color,[]);return f(()=>{let n=p(.58,.96,s.p);t.setRGB(a(.02,.93,n),a(.105,.96,n),a(.145,.962,n)),e.current.color.copy(t)}),i(\"fog\",{ref:e,attach:\"fog\",args:[\"#0a2732\",14,62]})}function te(){let e=R();return f(()=>{window.__three=e}),null}function oe({dof:e,vignette:t}){let{camera:n}=R();return f(()=>{let o=s.p;n.position.y=0,n.position.z=o<=.5?a(z,0,o/.5):a(0,k,(o-.5)/.5),n.updateMatrixWorld();let c=C(o),r=b(o/.5,0,1),l=b((o-.5)/.5,0,1),d=o<=.5?c.z+a(2.8,0,r):c.z;if(e.current){let u=e.current.cocMaterial;u.worldFocusDistance=Math.max(Math.abs(n.position.z-d),.6),u.worldFocusRange=o<=.5?a(2.4,2.6,r):a(2.6,3.6,l)}t.current&&(t.current.darkness=a(.3,.26,p(.58,.96,o)))}),null}function ne(){let e=h(),t=h();return w(ae,{children:[i(ee,{}),i(te,{}),i(oe,{dof:e,vignette:t}),i(Y,{}),i($,{}),w(F,{disableNormalPass:!0,multisampling:0,children:[i(I,{ref:e,worldFocusDistance:6.4,worldFocusRange:1.8,bokehScale:5.5,height:640}),i(V,{mipmapBlur:!0,luminanceThreshold:.95,luminanceSmoothing:.3,intensity:.55}),i(H,{opacity:.075,blendFunction:W.OVERLAY}),i(G,{ref:t,eskil:!1,offset:.3,darkness:.3})]})]})}S(document.getElementById(\"scene\")).render(i(L,{dpr:[1,1.4],gl:{antialias:!1,powerPreference:\"high-performance\"},camera:{fov:38,near:.35,far:200,position:[0,0,0]},children:i(ne,{})}));\n"
...
export default function mount(host) {
  ...
  /* THE DOM IS OWNED, like the stop and the driver before it. Teardown used
     to wipe host.innerHTML unconditionally - so when a stale instance's
     cleanup ran AFTER a soft-refresh remount (the same reversed effect
     ordering as the cross-instance driver murder), it deleted the NEW
     instance's markup: text gone, a surviving canvas showing only the dot,
     and a controller holding detached elements - unresponsive to everything.
     Every destructive teardown step now checks this marker first. */
  const instId = String(Math.floor(performance.now() * 1000)) + "-" + MARKUP.length
  host.dataset.eviiveInst = instId
  ...
  return () => {
    ...
    if (host.dataset.eviiveInst !== instId) {
      /* A newer mount owns this DOM now - remove only what is unambiguously
         ours and leave the living instance alone. */
      try { hint.remove(); style.remove(); driverEl.remove() } catch (e) {}
      return
    }
    delete host.dataset.eviiveInst
    hint.remove()
