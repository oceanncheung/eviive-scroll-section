...
const DRIVER = "window.__eviiveStop&&window.__eviiveStop();let __eviiveDead=!1;window.__eviiveStop=function(){__eviiveDead=!0},(()=>{\"use strict\";const Kt=matchMedia(\"(prefers-reduced-motion: reduce)\").matches,v=(t,n,e)=>t<n?n:t>e?e:t,M=(t,n,e)=>t+(n-t)*e,k=(t,n,e)=>{const o=v((e-t)/(n-t),0,1);return o*o*(3-2*o)},Nt=6.4,Xt=53.5,i=window.__eviiveScroll=window.__eviiveScroll||{p:0};i.mx=i.my=0,addEventListener(\"pointermove\",t=>{i.mx=t.clientX/__eviiveVP.w*2-1,i.my=1-t.clientY/__eviiveVP.h*2},{passive:!0});const Ut=document.getElementById(\"pin\"),H=document.getElementById(\"ui\"),q=document.getElementById(\"line2\"),W=document.getElementById(\"l1a\"),Y=document.getElementById(\"l1b\"),m=document.getElementById(\"caption\"),zt=document.getElementById(\"cue\"),Et=document.getElementById(\"hud\"),Z=document.getElementById(\"odo\"),Zt=[{key:\"tens\",mul:.1},{key:\"ones\",mul:1},{key:\"point\"},{key:\"tenths\",mul:10}],O={},ct={},G={};let at=102;for(const t of Zt){if(t.key===\"point\"){const o=document.createElement(\"div\");o.className=\"odo-pt\",o.textContent=\".\",Z.appendChild(o);continue}const n=document.createElement(\"div\");n.className=\"odo-col\";const e=document.createElement(\"div\");e.className=\"stack\";for(let o=0;o<=44;o++){const c=document.createElement(\"i\");c.textContent=String(o%10),e.appendChild(c)}n.appendChild(e),Z.appendChild(n),O[t.key]={st:e,col:n,mul:t.mul},G[t.key]=0}function rt(){O.ones&&(at=O.ones.col.getBoundingClientRect().height||at)}rt(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(rt),addEventListener(\"resize\",rt,{passive:!0});const jt={tens:[0,0,5],ones:[0,6,23],tenths:[0,14,35]};function Jt(t,n){const e=jt[t];return n<=.5?M(e[0],e[1],n/.5):M(e[1],e[2],(n-.5)/.5)}function Qt(t){for(const n in O){const{st:e,col:o}=O[n],c=Jt(n,t);e.style.transform=`translateY(${(.275-c).toFixed(4)}em)`;const r=ct[n]===void 0?c:ct[n];ct[n]=c;const s=Math.abs(c-r);G[n]=G[n]*.84+s*.16;const p=v(G[n]*30,0,1);o.style.setProperty(\"--eat\",(p*11.5).toFixed(2)+\"%\"),o.style.setProperty(\"--fade\",(2.4+p*11).toFixed(2)+\"%\");const u=v(G[n]*at*.3,0,9);e.style.filter=u>.3?`url(#mb-${n})`:\"none\";const l=document.getElementById(`fe-${n}`);if(l&&l.setAttribute(\"stdDeviation\",`0 ${u.toFixed(2)}`),n===\"tens\"){const y=k(.55,.78,t);o.style.opacity=y,o.style.width=(.6*y).toFixed(4)+\"em\"}}}let K=24;const E=()=>__eviiveVP.w/__eviiveVP.h<1.15||__eviiveVP.w<900;function te(){const t=g.getBoundingClientRect();K=Math.round(E()?t.left:__eviiveVP.w-t.right)}const Pt=22,Rt=3,lt=80,dt=30,pt=34,Ct=10,Bt=17,ke=32,ee=18,ne=10,Ve=[6,54],g=document.getElementById(\"rail\"),mt=[];for(let t=0;t<lt;t++){const n=document.createElement(\"i\");g.appendChild(n),mt.push(n)}const St=[{m:6,label:\"not aligned with prediction\"},{m:54,label:\"aligned with prediction\"}],At=St.map(t=>{const n=document.createElement(\"i\");return n.className=\"mark\",n.innerHTML=`<b></b><span>${t.label}</span>`,g.appendChild(n),n}),j=document.getElementById(\"odounit\"),C=document.querySelector(\".readout\"),f={top:0,h:1,left:0,w:1},J=[],Q={w:-1,h:-1},Ft=document.querySelector(\".readout .unit\");function kt(t){const n=t.style.whiteSpace;t.style.whiteSpace=\"nowrap\";const e=document.createElement(\"span\");e.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",t.appendChild(e);const o=e.getBoundingClientRect().bottom-t.getBoundingClientRect().top;return e.remove(),t.style.whiteSpace=n,o}function Vt(t){const n=getComputedStyle(t),e=document.createElement(\"span\");e.style.cssText=`position:absolute;left:-9999px;top:0;visibility:hidden;display:block;white-space:nowrap;font-family:${n.fontFamily};font-size:${n.fontSize};font-weight:${n.fontWeight};letter-spacing:${n.letterSpacing};line-height:${n.lineHeight}`,e.textContent=\"x\",document.body.appendChild(e);const o=document.createElement(\"span\");o.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",e.appendChild(o);const c=e.getBoundingClientRect().bottom-o.getBoundingClientRect().bottom;return e.remove(),c}const Lt=document.querySelector(\".sticky\");function tt(){if(__eviiveVP.h<100)return;Q.w=__eviiveVP.w,Q.h=__eviiveVP.h,i.stacked=E();var t=document.querySelector(\".eviive-root\")||document.documentElement;t.classList.toggle(\"stacked\",i.stacked),te(),m.style.left=m.style.right=m.style.bottom=\"\",m.style.textAlign=m.style.maxWidth=\"\",C.style.top=C.style.bottom=\"\",g.style.top=g.style.bottom=\"\";const n=parseFloat(getComputedStyle(document.querySelector(\".eviive-root\")||document.documentElement).getPropertyValue(\"--nav-h\"))||0;i.orbY=.5+n/(2*__eviiveVP.h),i.orbX=.5,i.orbDia=0;const e=Z.querySelectorAll(\".odo-col\"),o=e[e.length-1],c=o&&o.querySelector(\"i\");if(!c)return;const r=parseFloat(getComputedStyle(Z).fontSize);j.style.transform=\"none\";const s=o.getBoundingClientRect().top+.2744*r+kt(c),p=j.getBoundingClientRect().top+kt(j);j.style.transform=`translateY(${(s-p).toFixed(2)}px)`;const u=g.getBoundingClientRect();if(f.top=u.top,f.h=u.height,f.left=u.left,f.w=u.width,E()){m.style.transform=\"none\",C.style.transform=\"none\";const F=parseFloat(getComputedStyle(document.documentElement).getPropertyValue(\"--pad-b\"))||96,wt=40,bt=32,be=m.getBoundingClientRect().height,qt=__eviiveVP.w>809;m.style.bottom=F+\"px\",qt&&(m.style.left=\"auto\",m.style.right=K+\"px\",m.style.textAlign=\"right\",m.style.maxWidth=\"44%\");const Wt=qt?F:F+be+wt;C.style.top=\"auto\",C.style.bottom=Wt.toFixed(0)+\"px\";const Me=C.getBoundingClientRect().height;g.style.top=\"auto\",g.style.bottom=(Wt+Me+bt).toFixed(0)+\"px\";const z=g.getBoundingClientRect();f.top=z.top,f.h=z.height,f.left=z.left,f.w=z.width;const Ee=40,Pe=0,Re=.84,Ce=document.querySelector(\".headline\"),Be=[W,Y,q].map(Mt=>Mt.style.display);W.style.display=\"none\",Y.style.display=\"\",q.style.display=\"\";const Se=Ce.getBoundingClientRect().bottom;[W,Y,q].forEach((Mt,Fe)=>{Mt.style.display=Be[Fe]});const Yt=Lt?Lt.getBoundingClientRect().top:0,Ot=Se-Yt+Pe,Gt=z.top-Yt-Ee;i.orbY=(Ot+Gt)/2/__eviiveVP.h;const Ae=__eviiveVP.w-2*K;i.orbDia=Math.max(Math.min((Gt-Ot)*Re,Ae*.78,__eviiveVP.w*.34),0);return}const l=g.getBoundingClientRect().bottom;m.style.transform=\"none\";const y=m.getBoundingClientRect().bottom-Vt(m);m.style.transform=`translateY(${(l-y).toFixed(2)}px)`,C.style.transform=\"none\";const V=Ft.getBoundingClientRect().bottom-Vt(Ft);C.style.transform=`translateY(${(l-V).toFixed(2)}px)`,At.forEach((F,wt)=>{const bt=F.style.display;F.style.display=\"block\",J[wt]=F.lastElementChild.offsetWidth,F.style.display=bt});const P=document.querySelector(\".headline\"),w={right:P.offsetLeft+P.offsetWidth,bottom:P.offsetTop+P.offsetHeight},B=__eviiveVP.w-K-30,a=Math.max(J[0]||0,J[1]||0)||200,h=Math.max(.55,Math.min(1,(__eviiveVP.w-900)/612));i.railK=h;const _=Math.round((dt+pt)*h);i.reachMin=_+Pt+8;const d=w.right+24,R=B-a-40,b=__eviiveVP.h/2/Math.tan(38*Math.PI/360)/6.4,I=__eviiveVP.w/2,S=(i.orbY||.5)*__eviiveVP.h,A=Math.max(0,S-w.bottom),T=I-(w.right-4),U=T>=0?Math.sqrt(T*T+A*A):A,D=B-i.reachMin-a-16-I,Ht=Math.min(U,D);i._dbg={A:Math.round(T),dy:Math.round(A),orbCy:Math.round(S),hlB:Math.round(w.bottom),hlR:Math.round(w.right),capL:Math.round(U),capR:Math.round(D),natR:Math.round(b),orbY:+(i.orbY||0).toFixed(4)},i.orbX=.5,i.orbDia=Ht>=b?0:Math.max(Ht*2,80)}function oe(t){f.top=g.offsetTop,f.h=g.offsetHeight,f.left=g.offsetLeft,f.w=g.offsetWidth;const n=E()?f.w:f.h,e=E()?__eviiveVP.w*.5-f.left:(i.orbY||.5)*__eviiveVP.h-f.top,c=Math.max(e,1)/Math.max(t,.05),r=i.p,s=__eviiveVP.h/2/Math.tan(38*Math.PI/360),p=__eviiveVP.w-K-30,u=(dt+pt)*(i.railK||1),l=k(.56,.86,r),y=[1-l,l],V=[];St.forEach((a,h)=>{const _=e+(a.m-t)*c,d=At[h],R=d.lastElementChild,L=Math.min(k(-60,20,_),1-k(n*.56,n*.78,_)),b=y[h]*L;if(d.style.display=b>.004?\"block\":\"none\",b<=.004)return;V.push({m:a.m,o:b});const I=J[h]||1,S=__eviiveVP.w*(i.orbX||.5)+((h===0?i.orbPxRestA:i.orbPxRestB)||s/6.4),A=p-u-S,T=Math.max((A-I)/2,24),U=u+T*(.8+.2*et(_,n)),D=E()?Ct+Bt+10+ee*et(_,n):Math.max(U-Pt,u+6);E()?(d.style.transform=`translate3d(${_.toFixed(1)}px,0,0)`,d.firstElementChild.style.transform=`scaleY(${(D/220).toFixed(4)})`,R.style.transform=`translate3d(-50%,${-(D+ne).toFixed(1)}px,0)`):(d.style.transform=`translate3d(0,${_.toFixed(1)}px,0)`,d.firstElementChild.style.transform=`scaleX(${(D/300).toFixed(4)})`,R.style.transform=`translate3d(${(-U).toFixed(1)}px,-50%,0)`),d.style.opacity=b.toFixed(3),R.style.filter=b>.995?\"none\":`blur(${((1-b)*20).toFixed(2)}px)`});const P=Math.min(Math.abs(r),Math.abs(r-.5),Math.abs(r-1))<.004;let w=0,B=0;for(;B<lt;w+=Rt){const a=e+(w-t)*c;if(a>n+12)break;if(a<-12)continue;const h=V.find(A=>Math.abs(A.m-w)<Rt*.5),_=h?h.o:0;if(_>.996)continue;const d=mt[B++];d._on!==1&&(d.style.display=\"block\",d._on=1);const R=Math.round(a*10)/10,L=Math.round(E()?Ct+Bt*et(a,n):(dt+pt*et(a,n))*(i.railK||1));(d._y!==R||d._w!==L)&&(d.style.transform=E()?`translate3d(${R}px,0,0) scaleY(${(L/64).toFixed(4)})`:`translate3d(0,${R}px,0) scaleX(${(L/64).toFixed(4)})`,d._y=R,d._w=L);const I=(P?a<-2||a>n+2?0:1:a<0?Math.max(0,1+a/12):a>n?Math.max(0,1-(a-n)/12):1)*(1-_),S=Math.round(I*50)/50;d._e!==S&&(d.style.opacity=S,d._e=S)}for(let a=B;a<lt;a++){const h=mt[a];h._on!==0&&(h.style.display=\"none\",h._on=0)}}const se=1.9;function et(t,n){const e=Math.min(Math.abs(t/n-.5)*2,1),o=Math.cos(e*Math.PI/2);return Math.pow(o*o,se)}function ie(t){const n=t.textContent.split(/(\\s+)/);t.textContent=\"\";const e=[];for(const o of n){if(!o.trim()&&o.includes(`\n`)){t.appendChild(document.createElement(\"br\"));continue}const c=document.createElement(\"span\");c.className=\"wd\";const r=document.createElement(\"b\");r.textContent=o,c.appendChild(r),t.appendChild(c),o.trim()&&e.push(r)}return e}const ce=new Map([W,Y,q].map(t=>[t,ie(t)])),nt=850,ut=100,ae=240,re=10,$t=10,ht=It(.44,0,.56,1),ft=[{els:[W],inAt:()=>x<=.5,armAt:()=>x===.5,armExact:!0,exitY:-1,canReverse:!0},{els:[Y,q],inAt:()=>x>=1,exitY:-1,exitReverse:!0}],yt=380,_t=.45;for(const t of ft)t.words=t.els.reduce((n,e)=>n.concat(ce.get(e)),[]),t.span=nt+ut*Math.max(t.words.length-1,0),t.maxD0=ut*Math.max(t.words.length-1,0),t.xspan=yt+t.maxD0*_t,t.t=0,t.x=0;function ot(){for(const t of ft){const n=t.els.map(r=>r.style.display);t.els.forEach(r=>{r.style.display=\"\"});let e=null,o=-1,c=0;for(const r of t.words){const s=Math.round(r.getBoundingClientRect().top);(e===null||Math.abs(s-e)>4)&&(o++,e=s,c=0),r._d0=o*ae+c*ut,c++}t.maxD0=Math.max(0,...t.words.map(r=>r._d0)),t.span=nt+t.maxD0,t.xspan=yt+t.maxD0*_t,t.els.forEach((r,s)=>{r.style.display=n[s]})}}let vt=-1,gt=0,N=16,st=1,xt=0;function le(t,n){N=vt<0?16:Math.min(n-vt,64),vt=n,x!==xt&&(st=x>xt?1:-1,xt=x);for(const e of ft){const o=e.inAt(),c=e.armAt?e.armAt():o,r=e.x>.02;o&&!e.was&&(e.x=0),c&&!e.wasArmed&&(e.revEntry=r,e.t=0,e.x=0),e.was=o,e.wasArmed=c,o?(e.x=v(e.x-N/e.xspan,0,1),c&&(e.t=v(e.t+N/e.span,0,1))):(e.x=v(e.x+N/e.xspan,0,1),e.x>=1&&(e.t=0));for(const l of e.els)l._clr!==1&&(l.style.transform=\"none\",l.style.opacity=\"\",l.style.filter=\"none\",l._clr=1);const s=e.x>1e-4,p=s?e.x<.999:e.t>5e-4;for(const l of e.els){const y=p?\"\":\"none\";l._d!==y&&(l.style.display=y,l._d=y)}if(!p)continue;const u=s?e.x*e.xspan:e.t*e.span;for(let l=0;l<e.words.length;l++){const y=e.words[l],V=y._d0||0,P=st<0?e.maxD0-V:V,w=s?1-ht(v((u-P*_t)/yt,0,1)):ht(v((u-P)/nt,0,1)),B=s?-st:st,a=y,h=Math.round(w*100)/100;a._o!==h&&(a.style.opacity=h,a._o=h);const _=Math.round((1-w)*re*B*4)/4;a._y!==_&&(a.style.transform=Math.abs(_)<.2?\"none\":`translate3d(0,${_}px,0)`,a._y=_);const d=Math.round((1-w)*$t*4)/4;a._b!==d&&(a.style.filter=d<.2?\"none\":`blur(${d}px)`,a._b=d)}}}const it=(t,n,e)=>`rgba(${Math.round(M(t[0],n[0],e))},${Math.round(M(t[1],n[1],e))},${Math.round(M(t[2],n[2],e))},${M(t[3],n[3],e).toFixed(3)})`,de=[255,255,255,.96],pe=[11,32,40,.94],me=[248,246,242,.42],ue=[11,32,40,.52],he=[248,246,242,.22],fe=[11,32,40,.24],ye=[103,234,255,1],_e=[6,132,177,1];function ve(t){H.style.setProperty(\"--ink\",it(de,pe,t)),H.style.setProperty(\"--soft\",it(me,ue,t)),H.style.setProperty(\"--ghost\",it(he,fe,t)),H.style.setProperty(\"--accent\",it(ye,_e,t))}function It(t,n,e,o){const c=s=>{const p=1-s;return 3*p*p*s*t+3*p*s*s*e+s*s*s},r=s=>{const p=1-s;return 3*p*p*s*n+3*p*s*s*o+s*s*s};return s=>{if(s<=0)return 0;if(s>=1)return 1;let p=0,u=1,l=0;for(let y=0;y<24;y++)l=(p+u)/2,c(l)<s?p=l:u=l;return r((p+u)/2)}}const ge=It(.37,0,.63,1),xe=2600;let x=0,$=0,X=-1e9;const we=.75;function Tt(){if(window.__eviiveDriving||window.__eviiveOwned)return;const t=Ut.getBoundingClientRect(),n=t.height-__eviiveVP.h;if(t.top>0){const c=v(1-t.top/__eviiveVP.h,0,1);x=$=i.p=v(c/we,0,1)*.5,X=-1e9;return}const e=n>0?v(-t.top/n,0,1):0,o=.5+Math.round(e)/2;o!==x&&($=i.p,x=o,X=performance.now())}tt(),ot(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(()=>{tt(),ot()}),addEventListener(\"resize\",()=>{tt(),ot()}),addEventListener(\"scroll\",Tt,{passive:!0}),Tt();function Dt(t){if(!__eviiveDead){requestAnimationFrame(Dt);try{window.__eviiveTick=(window.__eviiveTick|0)+1,i.p=Kt?x:$+(x-$)*ge(v((t-X)/xe,0,1));const n=i.p;try{(Q.w!==__eviiveVP.w||Q.h!==__eviiveVP.h)&&(tt(),ot()),Qt(n),oe(n<=.5?M(0,6,n/.5):Math.exp(M(Math.log(6),Math.log(54),(n-.5)/.5))),le(n,t),ve(k(.58,.96,n))}catch(o){window.__eviiveErr||(window.__eviiveErr=o.stack||String(o),console.error(\"[eviive] frame:\",o))}H.style.opacity=k(.02,.14,n),gt=v(gt+(x>=1?1:-1)*N/nt,0,1);const e=ht(gt);m.style.opacity=(e*.9).toFixed(3),m.style.filter=e>.995?\"none\":`blur(${((1-e)*$t).toFixed(2)}px)`,zt.style.opacity=1-k(.02,.18,n),Et.classList.contains(\"on\")&&(Et.textContent=`p       ${n.toFixed(3)}\ncamZ    ${(-47.1*n).toFixed(2)}\nmonths  ${M(Nt,Xt,n).toFixed(1)}`)}catch(n){window.__eviiveErr||(window.__eviiveErr=n.stack||String(n),console.error(\"[eviive] frame:\",n))}}}requestAnimationFrame(Dt),window.__eviive={to(t){return $=i.p,x=t,X=performance.now(),t},seek(t){return x=$=i.p=v(t,0,1),X=-1e9,i.p},get progress(){return i.p},states:{a:0,b:1}}})();\n"
