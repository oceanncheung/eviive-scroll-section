...
const DRIVER = "window.__eviiveStop&&window.__eviiveStop();let __eviiveDead=!1;window.__eviiveStop=function(){__eviiveDead=!0},(()=>{\"use strict\";const $t=matchMedia(\"(prefers-reduced-motion: reduce)\").matches,x=(t,n,e)=>t<n?n:t>e?e:t,E=(t,n,e)=>t+(n-t)*e,V=(t,n,e)=>{const o=x((e-t)/(n-t),0,1);return o*o*(3-2*o)},kt=6.4,Dt=53.5,a=window.__eviiveScroll=window.__eviiveScroll||{p:0};a.mx=a.my=0,addEventListener(\"pointermove\",t=>{a.mx=t.clientX/__eviiveVP.w*2-1,a.my=1-t.clientY/__eviiveVP.h*2},{passive:!0});const Tt=document.getElementById(\"pin\"),H=document.getElementById(\"ui\"),q=document.getElementById(\"line2\"),O=document.getElementById(\"l1a\"),W=document.getElementById(\"l1b\"),m=document.getElementById(\"caption\"),Ht=document.getElementById(\"cue\"),vt=document.getElementById(\"hud\"),z=document.getElementById(\"odo\"),qt=[{key:\"tens\",mul:.1},{key:\"ones\",mul:1},{key:\"point\"},{key:\"tenths\",mul:10}],Y={},st={},B={};let it=102;for(const t of qt){if(t.key===\"point\"){const o=document.createElement(\"div\");o.className=\"odo-pt\",o.textContent=\".\",z.appendChild(o);continue}const n=document.createElement(\"div\");n.className=\"odo-col\";const e=document.createElement(\"div\");e.className=\"stack\";for(let o=0;o<=44;o++){const i=document.createElement(\"i\");i.textContent=String(o%10),e.appendChild(i)}n.appendChild(e),z.appendChild(n),Y[t.key]={st:e,col:n,mul:t.mul},B[t.key]=0}function ct(){Y.ones&&(it=Y.ones.col.getBoundingClientRect().height||it)}ct(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(ct),addEventListener(\"resize\",ct,{passive:!0});const Ot={tens:[0,0,5],ones:[0,6,23],tenths:[0,14,35]};function Wt(t,n){const e=Ot[t];return n<=.5?E(e[0],e[1],n/.5):E(e[1],e[2],(n-.5)/.5)}function Yt(t){for(const n in Y){const{st:e,col:o}=Y[n],i=Wt(n,t);e.style.transform=`translateY(${(.275-i).toFixed(4)}em)`;const l=st[n]===void 0?i:st[n];st[n]=i;const s=Math.abs(i-l);B[n]=s>B[n]?s:B[n]*.55+s*.45,B[n]<1e-4&&(B[n]=0);const p=x(B[n]*30,0,1);o.style.setProperty(\"--eat\",(p*11.5).toFixed(2)+\"%\"),o.style.setProperty(\"--fade\",(2.4+p*11).toFixed(2)+\"%\");const h=x(B[n]*it*.3,0,9);e.style.filter=h>.1?`url(#mb-${n})`:\"none\";const r=document.getElementById(`fe-${n}`);if(r&&r.setAttribute(\"stdDeviation\",`0 ${h.toFixed(2)}`),n===\"tens\"){const f=V(.55,.78,t);o.style.opacity=f,o.style.width=(.6*f).toFixed(4)+\"em\"}}}let G=24;const C=()=>__eviiveVP.w/__eviiveVP.h<1.15||__eviiveVP.w<900;function Gt(){const t=v.getBoundingClientRect();G=Math.round(C()?t.left:__eviiveVP.w-t.right)}const Nt=22,wt=3,at=80,Ut=30,Xt=34,zt=10,Kt=17,Zt=32,jt=18,Jt=10,Fe=[6,54],v=document.getElementById(\"rail\"),lt=[];for(let t=0;t<at;t++){const n=document.createElement(\"i\");v.appendChild(n),lt.push(n)}const bt=[{m:6,label:\"not aligned with prediction\"},{m:54,label:\"aligned with prediction\"}],Mt=bt.map(t=>{const n=document.createElement(\"i\");return n.className=\"mark\",n.innerHTML=`<b></b><span>${t.label}</span>`,v.appendChild(n),n}),K=document.getElementById(\"odounit\"),S=document.querySelector(\".readout\"),u={top:0,h:1,left:0,w:1},Z=[],j={w:-1,h:-1},Et=document.querySelector(\".readout .unit\");function Ct(t){const n=t.style.whiteSpace;t.style.whiteSpace=\"nowrap\";const e=document.createElement(\"span\");e.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",t.appendChild(e);const o=e.getBoundingClientRect().bottom-t.getBoundingClientRect().top;return e.remove(),t.style.whiteSpace=n,o}function Rt(t){const n=getComputedStyle(t),e=document.createElement(\"span\");e.style.cssText=`position:absolute;left:-9999px;top:0;visibility:hidden;display:block;white-space:nowrap;font-family:${n.fontFamily};font-size:${n.fontSize};font-weight:${n.fontWeight};letter-spacing:${n.letterSpacing};line-height:${n.lineHeight}`,e.textContent=\"x\",document.body.appendChild(e);const o=document.createElement(\"span\");o.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",e.appendChild(o);const i=e.getBoundingClientRect().bottom-o.getBoundingClientRect().bottom;return e.remove(),i}function J(){j.w=__eviiveVP.w,j.h=__eviiveVP.h,a.stacked=C();var t=document.querySelector(\".eviive-root\")||document.documentElement;t.classList.toggle(\"stacked\",a.stacked),Gt(),m.style.left=m.style.right=m.style.bottom=\"\",m.style.textAlign=m.style.maxWidth=\"\",S.style.top=S.style.bottom=\"\",v.style.top=v.style.bottom=\"\";const n=parseFloat(getComputedStyle(document.querySelector(\".eviive-root\")||document.documentElement).getPropertyValue(\"--nav-h\"))||0;a.orbY=.5+n/(2*__eviiveVP.h),a.orbX=.5,a.orbDia=0;const e=z.querySelectorAll(\".odo-col\"),o=e[e.length-1],i=o&&o.querySelector(\"i\");if(!i)return;const l=parseFloat(getComputedStyle(z).fontSize);K.style.transform=\"none\";const s=o.getBoundingClientRect().top+.2744*l+Ct(i),p=K.getBoundingClientRect().top+Ct(K);K.style.transform=`translateY(${(s-p).toFixed(2)}px)`;const h=v.getBoundingClientRect();if(u.top=h.top,u.h=h.height,u.left=h.left,u.w=h.width,C()){m.style.transform=\"none\",S.style.transform=\"none\";const I=parseFloat(getComputedStyle(document.documentElement).getPropertyValue(\"--pad-b\"))||96,_t=40,gt=32,we=m.getBoundingClientRect().height,Ft=__eviiveVP.w>809;m.style.bottom=I+\"px\",Ft&&(m.style.left=\"auto\",m.style.right=G+\"px\",m.style.textAlign=\"right\",m.style.maxWidth=\"44%\");const Lt=Ft?I:I+we+_t;S.style.top=\"auto\",S.style.bottom=Lt.toFixed(0)+\"px\";const be=S.getBoundingClientRect().height;v.style.top=\"auto\",v.style.bottom=(Lt+be+gt).toFixed(0)+\"px\";const X=v.getBoundingClientRect();u.top=X.top,u.h=X.height,u.left=X.left,u.w=X.width;const Me=40,Ee=0,Ce=.84,Re=document.querySelector(\".headline\"),Pe=[O,W,q].map(xt=>xt.style.display);O.style.display=\"none\",W.style.display=\"\",q.style.display=\"\";const Ae=Re.getBoundingClientRect().bottom;[O,W,q].forEach((xt,Se)=>{xt.style.display=Pe[Se]});const It=Ae+Ee,Vt=X.top-Me;a.orbY=(It+Vt)/2/__eviiveVP.h;const Be=__eviiveVP.w-2*G;a.orbDia=Math.max(Math.min((Vt-It)*Ce,Be*.78),0);return}const r=v.getBoundingClientRect().bottom;m.style.transform=\"none\";const f=m.getBoundingClientRect().bottom-Rt(m);m.style.transform=`translateY(${(r-f).toFixed(2)}px)`,S.style.transform=\"none\";const $=Et.getBoundingClientRect().bottom-Rt(Et);S.style.transform=`translateY(${(r-$).toFixed(2)}px)`,Mt.forEach((I,_t)=>{const gt=I.style.display;I.style.display=\"block\",Z[_t]=I.lastElementChild.offsetWidth,I.style.display=gt});const R=document.querySelector(\".headline\"),w={right:R.offsetLeft+R.offsetWidth,bottom:R.offsetTop+R.offsetHeight},F=__eviiveVP.w-G-30,c=Math.max(Z[0]||0,Z[1]||0)||200,_=w.right+24,g=F-c-40,b=__eviiveVP.h/2/Math.tan(38*Math.PI/360)/6.4,P=__eviiveVP.w/2,M=(a.orbY||.5)*__eviiveVP.h,L=Math.max(0,M-w.bottom),A=P-(w.right-4),k=A>=0?Math.sqrt(A*A+L*L):L,T=F-c-P,St=Math.min(k,T);a._dbg={A:Math.round(A),dy:Math.round(L),orbCy:Math.round(M),hlB:Math.round(w.bottom),hlR:Math.round(w.right),capL:Math.round(k),capR:Math.round(T),natR:Math.round(b),orbY:+(a.orbY||0).toFixed(4)},a.orbX=.5,a.orbDia=St>=b?0:Math.max(St*2,80)}function Qt(t){u.top=v.offsetTop,u.h=v.offsetHeight,u.left=v.offsetLeft,u.w=v.offsetWidth;const n=C()?u.w:u.h,e=C()?__eviiveVP.w*.5-u.left:(a.orbY||.5)*__eviiveVP.h-u.top,i=Math.max(e,1)/Math.max(t,.05),l=a.p,s=__eviiveVP.h/2/Math.tan(38*Math.PI/360),p=__eviiveVP.w*(a.orbX||.5)+s/6.4,h=__eviiveVP.w-G-30,r=V(.56,.86,l),f=[1-r,r],$=[];bt.forEach((c,_)=>{const g=e+(c.m-t)*i,d=Mt[_],b=d.lastElementChild,P=Math.min(V(-60,20,g),1-V(n*.56,n*.78,g)),M=f[_]*P;if(d.style.display=M>.004?\"block\":\"none\",M<=.004)return;$.push({m:c.m,o:M});const L=Z[_]||1,k=30+Math.max((h-p-L)/2,40)*(.52+.48*Q(g,n)),T=C()?Zt+jt*Q(g,n):Math.max(k-Nt,24);C()?(d.style.transform=`translate3d(${g.toFixed(1)}px,0,0)`,d.firstElementChild.style.transform=`scaleY(${(T/220).toFixed(4)})`,b.style.transform=`translate3d(-50%,${-(T+Jt).toFixed(1)}px,0)`):(d.style.transform=`translate3d(0,${g.toFixed(1)}px,0)`,d.firstElementChild.style.transform=`scaleX(${(T/300).toFixed(4)})`,b.style.transform=`translate3d(${(-k).toFixed(1)}px,-50%,0)`),d.style.opacity=M.toFixed(3),b.style.filter=M>.995?\"none\":`blur(${((1-M)*20).toFixed(2)}px)`});const R=Math.min(Math.abs(l),Math.abs(l-.5),Math.abs(l-1))<.004;let w=0,F=0;for(;F<at;w+=wt){const c=e+(w-t)*i;if(c>n+12)break;if(c<-12)continue;const _=$.find(k=>Math.abs(k.m-w)<wt*.5),g=_?_.o:0;if(g>.996)continue;const d=lt[F++];d._on!==1&&(d.style.display=\"block\",d._on=1);const b=Math.round(c*10)/10,P=Math.round(C()?zt+Kt*Q(c,n):Ut+Xt*Q(c,n));(d._y!==b||d._w!==P)&&(d.style.transform=C()?`translate3d(${b}px,0,0) scaleY(${(P/64).toFixed(4)})`:`translate3d(0,${b}px,0) scaleX(${(P/64).toFixed(4)})`,d._y=b,d._w=P);const L=(R?c<-2||c>n+2?0:1:c<0?Math.max(0,1+c/12):c>n?Math.max(0,1-(c-n)/12):1)*(1-g),A=Math.round(L*50)/50;d._e!==A&&(d.style.opacity=A,d._e=A)}for(let c=F;c<at;c++){const _=lt[c];_._on!==0&&(_.style.display=\"none\",_._on=0)}}const te=1.9;function Q(t,n){const e=Math.min(Math.abs(t/n-.5)*2,1),o=Math.cos(e*Math.PI/2);return Math.pow(o*o,te)}function ee(t){const n=t.textContent.split(/(\\s+)/);t.textContent=\"\";const e=[];for(const o of n){if(!o.trim()&&o.includes(`\n`)){t.appendChild(document.createElement(\"br\"));continue}const i=document.createElement(\"span\");i.className=\"wd\";const l=document.createElement(\"b\");l.textContent=o,i.appendChild(l),t.appendChild(i),o.trim()&&e.push(l)}return e}const ne=new Map([O,W,q].map(t=>[t,ee(t)])),tt=850,rt=100,oe=240,se=10,Pt=10,dt=fe(.44,0,.56,1),pt=[{els:[O],inAt:()=>y<=.5,armAt:()=>y===.5,armExact:!0,exitY:-1,canReverse:!0},{els:[W,q],inAt:()=>y>=1,exitY:-1,exitReverse:!0}],mt=380,ut=.45;for(const t of pt)t.words=t.els.reduce((n,e)=>n.concat(ne.get(e)),[]),t.span=tt+rt*Math.max(t.words.length-1,0),t.maxD0=rt*Math.max(t.words.length-1,0),t.xspan=mt+t.maxD0*ut,t.t=0,t.x=0;function et(){for(const t of pt){const n=t.els.map(l=>l.style.display);t.els.forEach(l=>{l.style.display=\"\"});let e=null,o=-1,i=0;for(const l of t.words){const s=Math.round(l.getBoundingClientRect().top);(e===null||Math.abs(s-e)>4)&&(o++,e=s,i=0),l._d0=o*oe+i*rt,i++}t.maxD0=Math.max(0,...t.words.map(l=>l._d0)),t.span=tt+t.maxD0,t.xspan=mt+t.maxD0*ut,t.els.forEach((l,s)=>{l.style.display=n[s]})}}let ht=-1,ft=0,N=16,nt=1,yt=0;function ie(t,n){N=ht<0?16:Math.min(n-ht,64),ht=n,y!==yt&&(nt=y>yt?1:-1,yt=y);for(const e of pt){const o=e.inAt(),i=e.armAt?e.armAt():o,l=e.x>.02;o&&!e.was&&(e.x=0),i&&!e.wasArmed&&(e.revEntry=l,e.t=0,e.x=0),e.was=o,e.wasArmed=i,o?(e.x=x(e.x-N/e.xspan,0,1),i&&(e.t=x(e.t+N/e.span,0,1))):(e.x=x(e.x+N/e.xspan,0,1),e.x>=1&&(e.t=0));for(const r of e.els)r._clr!==1&&(r.style.transform=\"none\",r.style.opacity=\"\",r.style.filter=\"none\",r._clr=1);const s=e.x>1e-4,p=s?e.x<.999:e.t>5e-4;for(const r of e.els){const f=p?\"\":\"none\";r._d!==f&&(r.style.display=f,r._d=f)}if(!p)continue;const h=s?e.x*e.xspan:e.t*e.span;for(let r=0;r<e.words.length;r++){const f=e.words[r],$=f._d0||0,R=nt<0?e.maxD0-$:$,w=s?1-dt(x((h-R*ut)/mt,0,1)):dt(x((h-R)/tt,0,1)),F=s?-nt:nt,c=f,_=Math.round(w*100)/100;c._o!==_&&(c.style.opacity=_,c._o=_);const g=Math.round((1-w)*se*F*4)/4;c._y!==g&&(c.style.transform=Math.abs(g)<.2?\"none\":`translate3d(0,${g}px,0)`,c._y=g);const d=Math.round((1-w)*Pt*4)/4;c._b!==d&&(c.style.filter=d<.2?\"none\":`blur(${d}px)`,c._b=d)}}}const ot=(t,n,e)=>`rgba(${Math.round(E(t[0],n[0],e))},${Math.round(E(t[1],n[1],e))},${Math.round(E(t[2],n[2],e))},${E(t[3],n[3],e).toFixed(3)})`,ce=[255,255,255,.96],ae=[11,32,40,.94],le=[248,246,242,.42],re=[11,32,40,.52],de=[248,246,242,.22],pe=[11,32,40,.24],me=[103,234,255,1],ue=[6,132,177,1];function he(t){H.style.setProperty(\"--ink\",ot(ce,ae,t)),H.style.setProperty(\"--soft\",ot(le,re,t)),H.style.setProperty(\"--ghost\",ot(de,pe,t)),H.style.setProperty(\"--accent\",ot(me,ue,t))}function fe(t,n,e,o){const i=s=>{const p=1-s;return 3*p*p*s*t+3*p*s*s*e+s*s*s},l=s=>{const p=1-s;return 3*p*p*s*n+3*p*s*s*o+s*s*s};return s=>{if(s<=0)return 0;if(s>=1)return 1;let p=0,h=1,r=0;for(let f=0;f<24;f++)r=(p+h)/2,i(r)<s?p=r:h=r;return l((p+h)/2)}}const ye=t=>{if(t>=1)return 1;const n=6*t;return 1-Math.exp(-n)*(1+n)},_e=900,ge=1300,xe=t=>t>=.75?ge:_e;let y=0,D=0,U=-1e9;const ve=.75;function At(){if(window.__eviiveDriving||window.__eviiveOwned)return;const t=Tt.getBoundingClientRect(),n=t.height-__eviiveVP.h;if(t.top>0){const i=x(1-t.top/__eviiveVP.h,0,1);y=D=a.p=x(i/ve,0,1)*.5,U=-1e9;return}const e=n>0?x(-t.top/n,0,1):0,o=.5+Math.round(e)/2;o!==y&&(D=a.p,y=o,U=performance.now())}J(),et(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(()=>{J(),et()}),addEventListener(\"resize\",()=>{J(),et()}),addEventListener(\"scroll\",At,{passive:!0}),At();function Bt(t){if(__eviiveDead)return;a.p=$t?y:D+(y-D)*ye(x((t-U)/xe(y),0,1));const n=a.p;try{(j.w!==__eviiveVP.w||j.h!==__eviiveVP.h)&&(J(),et()),Yt(n),Qt(n<=.5?E(0,6,n/.5):Math.exp(E(Math.log(6),Math.log(54),(n-.5)/.5))),ie(n,t),he(V(.58,.96,n))}catch(o){window.__eviiveErr||(window.__eviiveErr=o.stack||String(o),console.error(\"[eviive] frame:\",o))}H.style.opacity=V(.02,.14,n),ft=x(ft+(y>=1?1:-1)*N/tt,0,1);const e=dt(ft);m.style.opacity=(e*.9).toFixed(3),m.style.filter=e>.995?\"none\":`blur(${((1-e)*Pt).toFixed(2)}px)`,Ht.style.opacity=1-V(.02,.18,n),vt.classList.contains(\"on\")&&(vt.textContent=`p       ${n.toFixed(3)}\ncamZ    ${(-47.1*n).toFixed(2)}\nmonths  ${E(kt,Dt,n).toFixed(1)}`),requestAnimationFrame(Bt)}requestAnimationFrame(Bt),window.__eviive={to(t){return D=a.p,y=t,U=performance.now(),t},seek(t){return y=D=a.p=x(t,0,1),U=-1e9,a.p},get progress(){return a.p},states:{a:0,b:1}}})();\n"
...
export default function mount(host) {
  ...
  const onWheel = (e) => {
    ...
    if (!armed) {
      /* Same gesture, already decided - but if WE took it, its tail is ours
         ...
         arrived. It read as the section refusing to hold, and as the reveal
         never playing, because the reader was carried off mid-animation. */
      if (ours) e.preventDefault()
      return
    }
    accum += e.deltaY
    if (Math.abs(accum) < TOLERANCE) return
    if (!deliberate && seen() < RESCUE_IN) return   // still theirs to scroll
    /* Stay armed until the gesture actually TAKES something. Deciding once, on
       a gesture's first event, meant a hard flick that began before the section
       was visible waved its whole burst through and sailed past the section
       entirely - a hundred events, nothing consumed, three viewports of travel.
       A gesture may still only capture once (armed clears below, and its tail
       is swallowed by `ours`), but it gets to notice the section arriving
       part-way through, which is the difference between coming smoothly to rest
       and flying by. */
  ...
  const onTouchMove = (e) => {
    ...
    if (!deliberate && seen() < RESCUE_IN) return
