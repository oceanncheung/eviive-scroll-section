...
const DRIVER = "window.__eviiveStop&&window.__eviiveStop();let __eviiveDead=!1;window.__eviiveStop=function(){__eviiveDead=!0},(()=>{\"use strict\";const kt=matchMedia(\"(prefers-reduced-motion: reduce)\").matches,g=(t,n,e)=>t<n?n:t>e?e:t,E=(t,n,e)=>t+(n-t)*e,V=(t,n,e)=>{const o=g((e-t)/(n-t),0,1);return o*o*(3-2*o)},Tt=6.4,Dt=53.5,r=window.__eviiveScroll=window.__eviiveScroll||{p:0};r.mx=r.my=0,addEventListener(\"pointermove\",t=>{r.mx=t.clientX/__eviiveVP.w*2-1,r.my=1-t.clientY/__eviiveVP.h*2},{passive:!0});const Ht=document.getElementById(\"pin\"),T=document.getElementById(\"ui\"),D=document.getElementById(\"line2\"),H=document.getElementById(\"l1a\"),q=document.getElementById(\"l1b\"),m=document.getElementById(\"caption\"),qt=document.getElementById(\"cue\"),vt=document.getElementById(\"hud\"),U=document.getElementById(\"odo\"),Wt=[{key:\"tens\",mul:.1},{key:\"ones\",mul:1},{key:\"point\"},{key:\"tenths\",mul:10}],W={},st={},Y={};let it=102;for(const t of Wt){if(t.key===\"point\"){const o=document.createElement(\"div\");o.className=\"odo-pt\",o.textContent=\".\",U.appendChild(o);continue}const n=document.createElement(\"div\");n.className=\"odo-col\";const e=document.createElement(\"div\");e.className=\"stack\";for(let o=0;o<=44;o++){const i=document.createElement(\"i\");i.textContent=String(o%10),e.appendChild(i)}n.appendChild(e),U.appendChild(n),W[t.key]={st:e,col:n,mul:t.mul},Y[t.key]=0}function ct(){W.ones&&(it=W.ones.col.getBoundingClientRect().height||it)}ct(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(ct),addEventListener(\"resize\",ct,{passive:!0});const Yt={tens:[0,0,5],ones:[0,6,23],tenths:[0,14,35]};function Ot(t,n){const e=Yt[t];return n<=.5?E(e[0],e[1],n/.5):E(e[1],e[2],(n-.5)/.5)}function Gt(t){for(const n in W){const{st:e,col:o}=W[n],i=Ot(n,t);e.style.transform=`translateY(${(.275-i).toFixed(4)}em)`;const a=st[n]===void 0?i:st[n];st[n]=i;const s=Math.abs(i-a);Y[n]=Y[n]*.84+s*.16;const p=g(Y[n]*30,0,1);o.style.setProperty(\"--eat\",(p*11.5).toFixed(2)+\"%\"),o.style.setProperty(\"--fade\",(2.4+p*11).toFixed(2)+\"%\");const h=g(Y[n]*it*.3,0,9);e.style.filter=h>.3?`url(#mb-${n})`:\"none\";const l=document.getElementById(`fe-${n}`);if(l&&l.setAttribute(\"stdDeviation\",`0 ${h.toFixed(2)}`),n===\"tens\"){const f=V(.55,.78,t);o.style.opacity=f,o.style.width=(.6*f).toFixed(4)+\"em\"}}}let O=24;const C=()=>__eviiveVP.w/__eviiveVP.h<1.15||__eviiveVP.w<900;function Nt(){const t=x.getBoundingClientRect();O=Math.round(C()?t.left:__eviiveVP.w-t.right)}const Xt=22,bt=3,at=80,Ut=30,zt=34,Kt=10,Zt=17,jt=32,Jt=18,Qt=10,Be=[6,54],x=document.getElementById(\"rail\"),lt=[];for(let t=0;t<at;t++){const n=document.createElement(\"i\");x.appendChild(n),lt.push(n)}const wt=[{m:6,label:\"not aligned with prediction\"},{m:54,label:\"aligned with prediction\"}],Mt=wt.map(t=>{const n=document.createElement(\"i\");return n.className=\"mark\",n.innerHTML=`<b></b><span>${t.label}</span>`,x.appendChild(n),n}),z=document.getElementById(\"odounit\"),B=document.querySelector(\".readout\"),u={top:0,h:1,left:0,w:1},K=[],Z={w:-1,h:-1},Et=document.querySelector(\".readout .unit\");function Ct(t){const n=t.style.whiteSpace;t.style.whiteSpace=\"nowrap\";const e=document.createElement(\"span\");e.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",t.appendChild(e);const o=e.getBoundingClientRect().bottom-t.getBoundingClientRect().top;return e.remove(),t.style.whiteSpace=n,o}function Rt(t){const n=getComputedStyle(t),e=document.createElement(\"span\");e.style.cssText=`position:absolute;left:-9999px;top:0;visibility:hidden;display:block;white-space:nowrap;font-family:${n.fontFamily};font-size:${n.fontSize};font-weight:${n.fontWeight};letter-spacing:${n.letterSpacing};line-height:${n.lineHeight}`,e.textContent=\"x\",document.body.appendChild(e);const o=document.createElement(\"span\");o.style.cssText=\"display:inline-block;width:0;height:0;vertical-align:baseline\",e.appendChild(o);const i=e.getBoundingClientRect().bottom-o.getBoundingClientRect().bottom;return e.remove(),i}function j(){Z.w=__eviiveVP.w,Z.h=__eviiveVP.h,r.stacked=C();var t=document.querySelector(\".eviive-root\")||document.documentElement;t.classList.toggle(\"stacked\",r.stacked),Nt(),m.style.left=m.style.right=m.style.bottom=\"\",m.style.textAlign=m.style.maxWidth=\"\",B.style.top=B.style.bottom=\"\",x.style.top=x.style.bottom=\"\";const n=parseFloat(getComputedStyle(document.querySelector(\".eviive-root\")||document.documentElement).getPropertyValue(\"--nav-h\"))||0;r.orbY=.5+n/(2*__eviiveVP.h),r.orbX=.5,r.orbDia=0;const e=U.querySelectorAll(\".odo-col\"),o=e[e.length-1],i=o&&o.querySelector(\"i\");if(!i)return;const a=parseFloat(getComputedStyle(U).fontSize);z.style.transform=\"none\";const s=o.getBoundingClientRect().top+.2744*a+Ct(i),p=z.getBoundingClientRect().top+Ct(z);z.style.transform=`translateY(${(s-p).toFixed(2)}px)`;const h=x.getBoundingClientRect();if(u.top=h.top,u.h=h.height,u.left=h.left,u.w=h.width,C()){m.style.transform=\"none\",B.style.transform=\"none\";const L=parseFloat(getComputedStyle(document.documentElement).getPropertyValue(\"--pad-b\"))||96,_t=40,gt=32,xe=m.getBoundingClientRect().height,Lt=__eviiveVP.w>809;m.style.bottom=L+\"px\",Lt&&(m.style.left=\"auto\",m.style.right=O+\"px\",m.style.textAlign=\"right\",m.style.maxWidth=\"44%\");const Vt=Lt?L:L+xe+_t;B.style.top=\"auto\",B.style.bottom=Vt.toFixed(0)+\"px\";const ve=B.getBoundingClientRect().height;x.style.top=\"auto\",x.style.bottom=(Vt+ve+gt).toFixed(0)+\"px\";const X=x.getBoundingClientRect();u.top=X.top,u.h=X.height,u.left=X.left,u.w=X.width;const be=40,we=0,Me=.84,Ee=document.querySelector(\".headline\"),Ce=[H,q,D].map(xt=>xt.style.display);H.style.display=\"none\",q.style.display=\"\",D.style.display=\"\";const Re=Ee.getBoundingClientRect().bottom;[H,q,D].forEach((xt,Ae)=>{xt.style.display=Ce[Ae]});const $t=Re+we,It=X.top-be;r.orbY=($t+It)/2/__eviiveVP.h;const Pe=__eviiveVP.w-2*O;r.orbDia=Math.max(Math.min((It-$t)*Me,Pe*.78),0);return}const l=x.getBoundingClientRect().bottom;m.style.transform=\"none\";const f=m.getBoundingClientRect().bottom-Rt(m);m.style.transform=`translateY(${(l-f).toFixed(2)}px)`,B.style.transform=\"none\";const $=Et.getBoundingClientRect().bottom-Rt(Et);B.style.transform=`translateY(${(l-$).toFixed(2)}px)`,Mt.forEach((L,_t)=>{const gt=L.style.display;L.style.display=\"block\",K[_t]=L.lastElementChild.offsetWidth,L.style.display=gt});const R=document.querySelector(\".headline\"),b={right:R.offsetLeft+R.offsetWidth,bottom:R.offsetTop+R.offsetHeight},S=__eviiveVP.w-O-30,c=Math.max(K[0]||0,K[1]||0)||200,y=b.right+24,_=S-c-40,w=__eviiveVP.h/2/Math.tan(38*Math.PI/360)/6.4,P=__eviiveVP.w/2,M=(r.orbY||.5)*__eviiveVP.h,F=Math.max(0,M-b.bottom),A=P-(b.right-4),I=A>=0?Math.sqrt(A*A+F*F):F,k=S-c-P,Ft=Math.min(I,k);r._dbg={A:Math.round(A),dy:Math.round(F),orbCy:Math.round(M),hlB:Math.round(b.bottom),hlR:Math.round(b.right),capL:Math.round(I),capR:Math.round(k),natR:Math.round(w),orbY:+(r.orbY||0).toFixed(4)},r.orbX=.5,r.orbDia=Ft>=w?0:Math.max(Ft*2,80)}function te(t){u.top=x.offsetTop,u.h=x.offsetHeight,u.left=x.offsetLeft,u.w=x.offsetWidth;const n=C()?u.w:u.h,e=C()?__eviiveVP.w*.5-u.left:(r.orbY||.5)*__eviiveVP.h-u.top,i=Math.max(e,1)/Math.max(t,.05),a=r.p,s=__eviiveVP.h/2/Math.tan(38*Math.PI/360),p=__eviiveVP.w*(r.orbX||.5)+s/6.4,h=__eviiveVP.w-O-30,l=V(.56,.86,a),f=[1-l,l],$=[];wt.forEach((c,y)=>{const _=e+(c.m-t)*i,d=Mt[y],w=d.lastElementChild,P=Math.min(V(-60,20,_),1-V(n*.56,n*.78,_)),M=f[y]*P;if(d.style.display=M>.004?\"block\":\"none\",M<=.004)return;$.push({m:c.m,o:M});const F=K[y]||1,I=30+Math.max((h-p-F)/2,40)*(.52+.48*J(_,n)),k=C()?jt+Jt*J(_,n):Math.max(I-Xt,24);C()?(d.style.transform=`translate3d(${_.toFixed(1)}px,0,0)`,d.firstElementChild.style.transform=`scaleY(${(k/220).toFixed(4)})`,w.style.transform=`translate3d(-50%,${-(k+Qt).toFixed(1)}px,0)`):(d.style.transform=`translate3d(0,${_.toFixed(1)}px,0)`,d.firstElementChild.style.transform=`scaleX(${(k/300).toFixed(4)})`,w.style.transform=`translate3d(${(-I).toFixed(1)}px,-50%,0)`),d.style.opacity=M.toFixed(3),w.style.filter=M>.995?\"none\":`blur(${((1-M)*20).toFixed(2)}px)`});const R=Math.min(Math.abs(a),Math.abs(a-.5),Math.abs(a-1))<.004;let b=0,S=0;for(;S<at;b+=bt){const c=e+(b-t)*i;if(c>n+12)break;if(c<-12)continue;const y=$.find(I=>Math.abs(I.m-b)<bt*.5),_=y?y.o:0;if(_>.996)continue;const d=lt[S++];d._on!==1&&(d.style.display=\"block\",d._on=1);const w=Math.round(c*10)/10,P=Math.round(C()?Kt+Zt*J(c,n):Ut+zt*J(c,n));(d._y!==w||d._w!==P)&&(d.style.transform=C()?`translate3d(${w}px,0,0) scaleY(${(P/64).toFixed(4)})`:`translate3d(0,${w}px,0) scaleX(${(P/64).toFixed(4)})`,d._y=w,d._w=P);const F=(R?c<-2||c>n+2?0:1:c<0?Math.max(0,1+c/12):c>n?Math.max(0,1-(c-n)/12):1)*(1-_),A=Math.round(F*50)/50;d._e!==A&&(d.style.opacity=A,d._e=A)}for(let c=S;c<at;c++){const y=lt[c];y._on!==0&&(y.style.display=\"none\",y._on=0)}}const ee=1.9;function J(t,n){const e=Math.min(Math.abs(t/n-.5)*2,1),o=Math.cos(e*Math.PI/2);return Math.pow(o*o,ee)}function ne(t){const n=t.textContent.split(/(\\s+)/);t.textContent=\"\";const e=[];for(const o of n){if(!o.trim()&&o.includes(`\n`)){t.appendChild(document.createElement(\"br\"));continue}const i=document.createElement(\"span\");i.className=\"wd\";const a=document.createElement(\"b\");a.textContent=o,i.appendChild(a),t.appendChild(i),o.trim()&&e.push(a)}return e}const oe=new Map([H,q,D].map(t=>[t,ne(t)])),Q=850,rt=100,se=240,ie=10,Pt=10,dt=At(.44,0,.56,1),pt=[{els:[H],inAt:()=>v<=.5,armAt:()=>v===.5,armExact:!0,exitY:-1,canReverse:!0},{els:[q,D],inAt:()=>v>=1,exitY:-1,exitReverse:!0}],mt=380,ut=.45;for(const t of pt)t.words=t.els.reduce((n,e)=>n.concat(oe.get(e)),[]),t.span=Q+rt*Math.max(t.words.length-1,0),t.maxD0=rt*Math.max(t.words.length-1,0),t.xspan=mt+t.maxD0*ut,t.t=0,t.x=0;function tt(){for(const t of pt){const n=t.els.map(a=>a.style.display);t.els.forEach(a=>{a.style.display=\"\"});let e=null,o=-1,i=0;for(const a of t.words){const s=Math.round(a.getBoundingClientRect().top);(e===null||Math.abs(s-e)>4)&&(o++,e=s,i=0),a._d0=o*se+i*rt,i++}t.maxD0=Math.max(0,...t.words.map(a=>a._d0)),t.span=Q+t.maxD0,t.xspan=mt+t.maxD0*ut,t.els.forEach((a,s)=>{a.style.display=n[s]})}}let ht=-1,ft=0,G=16,et=1,yt=0;function ce(t,n){G=ht<0?16:Math.min(n-ht,64),ht=n,v!==yt&&(et=v>yt?1:-1,yt=v);for(const e of pt){const o=e.inAt(),i=e.armAt?e.armAt():o,a=e.x>.02;o&&!e.was&&(e.x=0),i&&!e.wasArmed&&(e.revEntry=a,e.t=0,e.x=0),e.was=o,e.wasArmed=i,o?(e.x=g(e.x-G/e.xspan,0,1),i&&(e.t=g(e.t+G/e.span,0,1))):(e.x=g(e.x+G/e.xspan,0,1),e.x>=1&&(e.t=0));for(const l of e.els)l._clr!==1&&(l.style.transform=\"none\",l.style.opacity=\"\",l.style.filter=\"none\",l._clr=1);const s=e.x>1e-4,p=s?e.x<.999:e.t>5e-4;for(const l of e.els){const f=p?\"\":\"none\";l._d!==f&&(l.style.display=f,l._d=f)}if(!p)continue;const h=s?e.x*e.xspan:e.t*e.span;for(let l=0;l<e.words.length;l++){const f=e.words[l],$=f._d0||0,R=et<0?e.maxD0-$:$,b=s?1-dt(g((h-R*ut)/mt,0,1)):dt(g((h-R)/Q,0,1)),S=s?-et:et,c=f,y=Math.round(b*100)/100;c._o!==y&&(c.style.opacity=y,c._o=y);const _=Math.round((1-b)*ie*S*4)/4;c._y!==_&&(c.style.transform=Math.abs(_)<.2?\"none\":`translate3d(0,${_}px,0)`,c._y=_);const d=Math.round((1-b)*Pt*4)/4;c._b!==d&&(c.style.filter=d<.2?\"none\":`blur(${d}px)`,c._b=d)}}}const nt=(t,n,e)=>`rgba(${Math.round(E(t[0],n[0],e))},${Math.round(E(t[1],n[1],e))},${Math.round(E(t[2],n[2],e))},${E(t[3],n[3],e).toFixed(3)})`,ae=[255,255,255,.96],le=[11,32,40,.94],re=[248,246,242,.42],de=[11,32,40,.52],pe=[248,246,242,.22],me=[11,32,40,.24],ue=[103,234,255,1],he=[6,132,177,1];function fe(t){T.style.setProperty(\"--ink\",nt(ae,le,t)),T.style.setProperty(\"--soft\",nt(re,de,t)),T.style.setProperty(\"--ghost\",nt(pe,me,t)),T.style.setProperty(\"--accent\",nt(ue,he,t))}function At(t,n,e,o){const i=s=>{const p=1-s;return 3*p*p*s*t+3*p*s*s*e+s*s*s},a=s=>{const p=1-s;return 3*p*p*s*n+3*p*s*s*o+s*s*s};return s=>{if(s<=0)return 0;if(s>=1)return 1;let p=0,h=1,l=0;for(let f=0;f<24;f++)l=(p+h)/2,i(l)<s?p=l:h=l;return a((p+h)/2)}}const ye=At(.37,0,.63,1),_e=2600;let v=0,N=0,ot=-1e9;const ge=.75;function Bt(){const t=Ht.getBoundingClientRect(),n=t.height-__eviiveVP.h;if(t.top>0){const i=g(1-t.top/__eviiveVP.h,0,1);v=N=r.p=g(i/ge,0,1)*.5,ot=-1e9;return}const e=n>0?g(-t.top/n,0,1):0,o=.5+Math.round(e)/2;o!==v&&(N=r.p,v=o,ot=performance.now())}j(),tt(),document.fonts&&document.fonts.ready&&document.fonts.ready.then(()=>{j(),tt()}),addEventListener(\"resize\",()=>{j(),tt()}),addEventListener(\"scroll\",Bt,{passive:!0}),Bt();function St(t){if(__eviiveDead)return;r.p=kt?v:N+(v-N)*ye(g((t-ot)/_e,0,1));const n=r.p;try{(Z.w!==__eviiveVP.w||Z.h!==__eviiveVP.h)&&(j(),tt()),Gt(n),te(n<=.5?E(0,6,n/.5):Math.exp(E(Math.log(6),Math.log(54),(n-.5)/.5))),ce(n,t),fe(V(.58,.96,n))}catch(o){window.__eviiveErr||(window.__eviiveErr=o.stack||String(o),console.error(\"[eviive] frame:\",o))}T.style.opacity=V(.02,.14,n),ft=g(ft+(v>=1?1:-1)*G/Q,0,1);const e=dt(ft);m.style.opacity=(e*.9).toFixed(3),m.style.filter=e>.995?\"none\":`blur(${((1-e)*Pt).toFixed(2)}px)`,qt.style.opacity=1-V(.02,.18,n),vt.classList.contains(\"on\")&&(vt.textContent=`p       ${n.toFixed(3)}\ncamZ    ${(-47.1*n).toFixed(2)}\nmonths  ${E(Tt,Dt,n).toFixed(1)}`),requestAnimationFrame(St)}requestAnimationFrame(St),window.__eviive={seek(t){return v=N=r.p=g(t,0,1),ot=-1e9,r.p},get progress(){return r.p},states:{a:0,b:1}}})();\n"
